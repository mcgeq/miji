...
pub struct TodoUpdate {
    ...
    pub subtask_order: Option<Option<i32>>,
}
...
impl TryFrom<TodoUpdate> for entity::todo::ActiveModel {
    ...
    fn try_from(value: TodoUpdate) -> Result<Self, Self::Error> {
        Ok(entity::todo::ActiveModel {
            ...
            location: value.location.map_or(ActiveValue::NotSet, ActiveValue::Set),
...
impl TodoUpdate {
    pub fn apply_to_model(self, model: &mut entity::todo::ActiveModel) {
        ...
        set_active_value_t!(model, self, completed_at);
        set_active_value_t!(model, self, assignee_id);
        set_active_value_t!(model, self, progress);
        set_active_value_t!(model, self, location);
        set_active_value_t!(model, self, owner_id);
        set_active_value_t!(model, self, is_archived);
        set_active_value_t!(model, self, is_pinned);
        set_active_value_t!(model, self, estimate_minutes);
        set_active_value_t!(model, self, reminder_count);
        set_active_value_t!(model, self, parent_id);
        set_active_value_t!(model, self, subtask_order);
