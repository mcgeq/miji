...
use crate::{
    dto::family_member::{FamilyMemberCreate, FamilyMemberSearchQuery, FamilyMemberUpdate},
...
impl FamilyMemberService {
    ...
    pub async fn search_family_members(
        ...
        if let Some(name) = &query.name {
            if !name.trim().is_empty() {
                conditions =
                    conditions.add(entity::family_member::Column::Name.contains(name.trim()));
            }
        }
        ...
        if let Some(email) = &query.email {
            if !email.trim().is_empty() {
                conditions =
                    conditions.add(entity::family_member::Column::Email.contains(email.trim()));
            }
        }
        ...
        let models = query_builder.all(db).await.map_err(AppError::from)?;
    ...
    pub async fn list_recent_family_members(
        ...
        let models =
            FamilyMemberEntity::find()
                .filter(entity::family_member::Column::CreatedAt.gte(
