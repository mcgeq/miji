<script setup lang="ts">
  ...
  const paymentMethods = ref<
    { paymentMethod: string; amount: number; count: number; percentage: number }[]
  >([]);

  // 成员数据 - 从API加载
  const memberData = ref<
    { name: string; totalPaid: number; totalOwed: number; netBalance: number; color: string }[]
  >([]);
  ...
  const monthlyTrends = ref<
    { month: string; income: number; expense: number; netIncome: number }[]
  >([]);
  ...
  const summary = computed(() => ({
    ...
    balance:
      topIncomeCategories.value.reduce((sum, cat) => sum + cat.amount, 0) -
...
<template>
  <div class="w-full min-h-screen p-4 sm:p-6 bg-gray-50 dark:bg-gray-900">
    <!-- 页面标题 -->
    <div
      class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-xl shadow-sm"
    >
      <div class="flex-1 min-w-0">
        <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-white mb-2 truncate">
          家庭财务统计
        </h1>
        <p class="text-sm text-gray-600 dark:text-gray-400">数据可视化分析与报表</p>
      </div>
      <div class="flex flex-wrap items-center gap-3 shrink-0">
        ...
        <Button variant="secondary" size="sm" @click="loadStatistics">刷新</Button>
        <Dropdown
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
      ...
      <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border-l-4 border-l-red-500">
        <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">总支出</div>
      ...
      <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border-l-4 border-l-green-500">
        <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">总收入</div>
        <div class="text-3xl font-semibold text-gray-900 dark:text-white mb-1">
          ¥{{ summary.totalIncome.toFixed(2) }}
        </div>
        <div class="text-xs text-gray-500 dark:text-gray-400">本期收入</div>
      </div>

      <!-- 结余 -->
      <div
        class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border-l-4"
        :class="summary.balance > 0 ? 'border-l-green-500' : summary.balance < 0 ? 'border-l-red-500' : 'border-l-blue-500'"
      >
        <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">结余</div>
        <div class="text-3xl font-semibold text-gray-900 dark:text-white mb-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <!-- 支付方式图表 -->
      <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
        <PaymentMethodChartsSwitcher :top-payment-methods="paymentMethods" :loading="loading" />
      </div>

      <!-- 成员贡献图 -->
      <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
        <MemberContributionChart :data="memberData" title="成员支付贡献度" height="400px" />
      </div>

      <!-- 债务关系图 -->
      <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
        <DebtRelationChart :family-ledger-serial-num="_currentLedgerSerialNum" />
      </div>
