<script setup lang="ts">
  ...
  // 使用预算统计 composable
  const { state, loadAllStats, refresh } = useBudgetStats();
...
<template>
  <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
      ...
      <div class="flex items-center gap-2 shrink-0">
        <Button
          ...
          @click="refresh"
        >
          <RefreshCw class="w-4 h-4" :class="{ 'animate-spin': state.loading }" />
        </Button>
        <Button variant="secondary" size="sm" :title="t('common.actions.back')" @click="goBack">
    ...
    <div
      v-if="state.error"
      class="flex flex-col items-center justify-center py-12 sm:py-16 px-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-xl mb-4"
    >
      <div class="text-5xl sm:text-6xl mb-4 text-red-500">!</div>
      <div class="text-base sm:text-lg text-red-600 dark:text-red-400 mb-4 text-center">
        {{ state.error }}
      </div>
      <Button variant="danger" size="md" @click="refresh">{{ t('common.actions.retry') }}</Button>
    </div>
