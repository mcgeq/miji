  ...
  >
      ...
      <div class="text-content">
        ...
        <ul class="space-y-2 text-sm text-gray-600">
          <li class="flex items-start">
            <component :is="CheckCircle" class="w-5 h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
            <span
              ><strong>待办提醒</strong>- 不错过重要事项</span
            >
          </li>
          <li class="flex items-start">
            <component :is="CheckCircle" class="w-5 h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
            <span
              ><strong>账单提醒</strong>- 避免逾期罚款</span
            >
          </li>
          <li class="flex items-start">
            <component :is="CheckCircle" class="w-5 h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
            <span
              ><strong>健康提醒</strong>- 关注身体变化</span
            >
          </li>
        </ul>
    ...
    <template #footer>
      <div class="flex justify-end space-x-3">
        <Button @click="handleSkip" :disabled="requesting">稍后设置</Button>
        <Button @click="handleRequest" :loading="requesting">授予权限</Button>
      </div>
    </template>
  </Modal>
</template>
...
<script setup lang="ts">
  ...
  const { requestPermission, error: permissionError } = useNotificationPermission();
