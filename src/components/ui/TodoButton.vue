...
// 基础样式
const baseClasses = 'inline-flex items-center justify-center border rounded-lg transition-all duration-200 outline-none';
  ...
  switch (props.size) {
    case 'small':
      return props.iconOnly
        ? 'p-1'
        : 'px-1.5 py-0.5 gap-1 text-[0.625rem]';
    case 'large':
      return props.iconOnly
        ? 'p-1.5'
        : 'px-3 py-1.5 gap-2 text-sm';
    default: // medium
      return props.iconOnly
        ? 'p-1'
        : 'px-2 py-1 gap-1.5 text-xs';
  }
});
...
// 焦点样式
const focusClasses = 'focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2';
...
<template>
  ...
  >
    ...
    <!-- 文本（非 icon-only 模式） -->
    <span
      v-if="props.text && !props.iconOnly"
      :class="textClasses"
    >
      {{ props.text }}
    </span>
