<script setup lang="ts">
  ...
  function navigateSubmenu(
    submenuItem: { name: string; title: string; path: string },
    event: Event,
  ) {
  ...
  >
    ...
    <div class="pt-6 pb-2 flex justify-center">
      <img
        src=""
        alt="avatar"
        class="w-8 h-8 rounded-full border-2 border-gray-300 dark:border-gray-700 shadow-md hover:scale-110 transition-transform duration-200"
      />
    </div>
    ...
    <nav class="flex-1 py-4 px-1.5">
      <ul class="list-none p-0 m-0">
        <li v-for="item in menu" :key="item.name" class="relative my-1">
          <!-- 菜单项 -->
          <div
            class="flex justify-center items-center p-2.5 rounded-lg cursor-pointer transition-all duration-200 relative group"
            :class="[
              isActive(item)
          ...
          >
            ...
            >
              <template v-for="(submenuItem, index) in item.submenu" :key="submenuItem.name">
                <li
                  class="w-9 h-9 flex justify-center items-center rounded-lg cursor-pointer transition-all duration-200 group"
                  :class="[
    ...
    >
      <LogOut
        class="w-5 h-5 text-gray-500 dark:text-gray-400 group-hover:text-red-500 dark:group-hover:text-red-400 group-hover:scale-110 group-active:scale-95 transition-all duration-200"
      />
    </button>
  </aside>
</template>
