...
function adjustTime(type: 'hour' | 'minute' | 'second', delta: number) {
  ...
  const currentValue = type === 'hour' ? props.hour : type === 'minute' ? props.minute : props.second;
  ...
  emit(`update:${type}` as any, newValue);
}
...
function handleNumpadConfirm() {
  ...
  if (!Number.isNaN(numValue)) {
    ...
    emit(`update:${activeTimeInput.value}` as any, clampedValue);
  }
