...
<template>
  <div>
    ...
    >
      <div class="text-xs text-[light-dark(#6b7280,#9ca3af)] mb-1">{{ quickSelectLabel }}：</div>
      <div class="flex flex-wrap gap-1">
        <button
          v-for="type in quickSelectTypes"
          :key="type.code"
          type="button"
          class="text-xs py-1 px-2 rounded-md border cursor-pointer transition-all duration-200 ease-in-out max-sm:py-0.5 max-sm:px-1.5 max-sm:text-[0.6875rem]"
          :class="[
            selectedValue === type.code
              ? 'bg-[light-dark(#dbeafe,rgba(59,130,246,0.1))] border-blue-500 text-blue-500'
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mt-1 text-sm text-red-500" role="alert" aria-live="polite">
      {{ errorMessage }}
    </div>

    <!-- 帮助文本 -->
    <div v-if="helpText" class="mt-2 text-xs text-[light-dark(#6b7280,#9ca3af)] text-right">
      {{ helpText }}
    </div>
  </div>
</template>
