<script setup lang="ts">
  ...
  const progressColor = computed(() => {
    ...
    // 超支 - 错误色（红色）
    if (p > 100) return 'rgb(239 68 68)'; // red-500

    // 预警区 - 错误色（红色）
    if (p >= props.colorThresholds.warning) return 'rgb(239 68 68)'; // red-500

    // 警告区 - 警告色（橙色）
    if (p >= props.colorThresholds.safe) return 'rgb(249 115 22)'; // orange-500
...
<template>
  <div class="w-full">
    <!-- 进度条容器 -->
    <div class="mb-2">
      <div class="relative w-full h-6 bg-gray-200 dark:bg-gray-700 rounded-xl overflow-hidden">
        <!-- 进度填充 -->
        <div
        ...
        <div
          ...
          class="absolute top-0 h-full w-0.5 z-10 -translate-x-px before:content-[''] before:absolute before:-top-1 before:left-1/2 before:-translate-x-1/2 before:border-l-4 before:border-r-4 before:border-t-4 before:border-l-transparent before:border-r-transparent before:border-t-gray-500"
          :style="{ left: `${threshold}%` }"
          :title="`预警阈值: ${threshold}%`"
        >
          <div class="w-0.5 h-full bg-gray-500 opacity-60" />
        </div>
      </div>
    </div>
    ...
    >
      <div class="flex items-center gap-2">
        <span class="font-medium text-gray-900 dark:text-gray-100">¥{{ formatAmount(used) }}</span>
    ...
    <div
      v-if="isExceeded"
      class="flex items-center gap-1 mt-1 py-1 px-2 bg-red-50 dark:bg-red-950 rounded-md text-xs text-red-500"
    >
