...
<template>
  <div :class="widthClass">
    <div
      class="flex gap-2 items-center max-sm:flex-col max-sm:items-stretch" :class="[
    ...
    >
      ...
      >
        <button
          ...
          type="button"
          class="py-1 px-3 border rounded-md text-sm cursor-pointer transition-all duration-200 bg-[var(--color-base-100)] text-[var(--color-base-content)] border-[var(--color-neutral)]" :class="[
            ...
            disabled && 'bg-[var(--color-base-200)] text-[var(--color-neutral-content)] cursor-not-allowed',
          ]"
          :disabled="disabled"
          :title="account.name"
    ...
    <!-- 帮助文本 -->
    <div v-if="helpText" class="mt-2 text-xs text-[var(--color-neutral-content)]">
      {{ helpText }}
    </div>
  </div>
</template>
