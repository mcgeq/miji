...
<template>
  <div class="w-full h-full flex flex-col gap-2 p-1 max-w-full overflow-hidden relative">
    <!-- 今日待办任务列表 - 支持滚动但隐藏滚动条 -->
    <div
      class="flex-1 overflow-y-auto overflow-x-hidden flex flex-col w-full max-w-full scrollbar-none"
    >
      <TodoList
    ...
    <Teleport to="body">
      ...
      >
        <div
          v-if="showModal"
          class="fixed inset-0 bg-black/60 dark:bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4 sm:p-2 overflow-y-auto isolate"
          @click="closeModal"
        >
          <Transition
          ...
          >
            ...
            >
              <div
                class="px-6 py-4 sm:px-4 sm:py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 shrink-0"
              >
              ...
              <div
                class="px-6 py-6 sm:px-4 sm:py-4 overflow-y-auto bg-white dark:bg-gray-800 flex-1 min-h-0"
              >
