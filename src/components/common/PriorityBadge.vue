...
<script setup lang="ts">
  ...
  const emit = defineEmits<(e: 'changePriority', serialNum: string, priority: Priority) => void>();
  ...
  const menuStyle = computed<Partial<CSSProperties>>(() => {
    ...
    const button = document.querySelector(
      `[data-priority-btn="${props.serialNum}"]`,
    ) as HTMLElement;
    if (!button) return {};
  ...
  const priorityLabel = computed(() => t(priorityKeyMap[normalizedPriority.value]));
  ...
  function getPriorityButtonClass() {
    ...
    const hoverClasses = ['hover:shadow-[0_10px_15px_rgba(0,0,0,0.25)]', 'hover:scale-125'];

    const disabledClasses = props.completed ? ['cursor-not-allowed', 'opacity-60'] : [];

    const urgentClasses =
      normalizedPriority.value === 'URGENT'
        ? [
            'animate-[urgent-pulse_2s_ease-in-out_infinite]',
  ...
  function getMenuOptionClass(p: Priority) {
    ...
    const stateClasses = isActive
      ? ['bg-[light-dark(#e5e7eb,#4b5563)]', 'font-semibold', 'text-[light-dark(#000,#fff)]']
      : [
