<script setup lang="ts">
  ...
  const todayRecord = computed(() => {
    return (
      periodStore.periodDailyRecords.find(r => r.date.startsWith(DateUtils.getTodayDate())) || null
    );
  });
...
<template>
  <div class="w-full flex flex-col md:flex-row gap-2 md:gap-3 p-2 md:p-3 overflow-hidden">
    ...
    <!-- 今日信息组件 -->
    <div class="w-full md:w-88 shrink-0 self-stretch min-h-0 flex flex-col">
      <PeriodTodayInfo
