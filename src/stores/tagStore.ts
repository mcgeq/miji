...
// 向后兼容的导出（保留旧接口）
/** @deprecated 使用 useTagStore() 代替 */
export const tagsStore = {
  get tags() {
    const store = useTagStore();
    return computed(() => {
      const map = new Map<string, Tags>();
      store.tags.forEach(tag => map.set(tag.serialNum, tag));
      return map;
    });
  },
  createTag: (input?: Partial<Tags>) => {
    const store = useTagStore();
  ...
  updateTag: (serialNum: string, input: Partial<Omit<Tags, 'serialNum' | 'createdAt'>>) => {
    const store = useTagStore();
    return store.updateTag(serialNum, input);
  },
  removeTag: (serialNum: string) => {
    const store = useTagStore();
    return store.deleteTag(serialNum).then(() => true);
  },
  clearAllTags: () => {
    const store = useTagStore();
    store.$reset();
  ...
  getAllTags: () => {
    const store = useTagStore();
    return store.tagsMap;
  },
  getTag: (serialNum: string) => {
    const store = useTagStore();
    return store.getTag(serialNum);
  },
  hasTag: (serialNum: string) => {
    const store = useTagStore();
    return store.hasTag(serialNum);
  },
  findTagsByName: (name: string) => {
    const store = useTagStore();
    return store.findTagsByName(name);
  },
  searchTags: (options: TagFilterOptions = {}) => {
    const store = useTagStore();
    return store.filterAndSortTags(options);
  },
};
