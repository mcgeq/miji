...
export function mapUIFiltersToAPIFilters(ui: UIFilters): BudgetFilters {
  return {
    ...
    startDate:
      ui.startDate?.start || ui.startDate?.end
        ? { ...ui.startDate }
        : undefined,
    endDate:
      ui.endDate?.start || ui.endDate?.end ? { ...ui.endDate } : undefined,
...
export function useBudgetFilters(budgets: () => Budget[], defaultPageSize = 4) {
  ...
  const filteredBudgets = computed(() => {
    ...
    if (filters.value.isActive === 'active')
      result = result.filter(b => b.isActive);
    else if (filters.value.isActive === 'inactive')
      result = result.filter(b => !b.isActive);

    if (filters.value.repeatPeriod) {
      result = result.filter(
        b => b.repeatPeriod.type === filters.value.repeatPeriod,
      );
    }
    if (filters.value.category) {
      result = result.filter(b =>
        b.categoryScope.includes(filters.value.category as Category),
      );
    }
  ...
  const pagination = usePagination<Budget>(
