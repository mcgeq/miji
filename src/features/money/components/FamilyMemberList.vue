...
function getRoleColor(role: string): string {
  ...
  return roleColors[role as keyof typeof roleColors] || 'text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800';
}
...
<template>
  <div class="p-4">
    ...
    <!-- 空状态 -->
    <div v-else-if="members.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
        ...
        <div class="flex items-start gap-3 mb-4">
          ...
          <div class="flex-1">
            ...
            <div class="flex items-center">
              <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="getRoleColor(member.role)">
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-3 mb-3">
          <div class="flex justify-between mb-2">
            <div class="flex flex-col items-center flex-1">
              ...
              <span class="text-sm font-medium text-gray-900 dark:text-white">¥{{ formatAmount(getMemberStats(member.serialNum).totalPaid) }}</span>
            </div>
            <div class="flex flex-col items-center flex-1">
              <span class="text-xs text-gray-500 dark:text-gray-400 mb-0.5">应分摊</span>
              <span class="text-sm font-medium text-gray-900 dark:text-white">¥{{ formatAmount(getMemberStats(member.serialNum).totalOwed) }}</span>
            </div>
          </div>
          ...
          <div class="flex justify-between mb-2">
            <div class="flex flex-col items-center flex-1">
              ...
              <span class="text-sm font-medium" :class="getBalanceStatus(getMemberStats(member.serialNum).netBalance).color">
          ...
          <div class="flex justify-between">
            <div class="flex flex-col items-center flex-1">
              ...
              <span class="text-sm font-medium text-gray-900 dark:text-white">{{ getMemberStats(member.serialNum).transactionCount }}</span>
            </div>
            <div class="flex flex-col items-center flex-1">
              <span class="text-xs text-gray-500 dark:text-gray-400 mb-0.5">分摊数</span>
              <span class="text-sm font-medium text-gray-900 dark:text-white">{{ getMemberStats(member.serialNum).splitCount }}</span>
            </div>
          </div>
        </div>

        <!-- 最后活跃时间 -->
        <div v-if="member.lastActiveAt" class="border-t border-gray-200 dark:border-gray-700 pt-2 text-center">
