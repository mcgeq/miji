  ...
  >
    <form @submit.prevent="save">
      ...
      <FormRow label="模板名称" required :error="errors.name">
        <Input v-model="form.name" type="text" placeholder="例如：家庭日常开销" />
      ...
      <FormRow label="分摊类型" required>
        <div class="grid grid-cols-2 md:grid-cols-2 gap-3">
          <label
            ...
            :key="type"
            class="px-3 py-2 border-2 rounded-lg text-center cursor-pointer transition-all"
            :class="[
              form.ruleType === type
          ...
          >
            <input v-model="form.ruleType" type="radio" :value="type" hidden />
      ...
      <!-- 设为默认 -->
      <div class="mb-3">
        <Checkbox v-model="form.isDefault" label="设为默认模板" />
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-6">新建交易时自动应用此模板</p>
      </div>
    </form>
  </Modal>
</template>
