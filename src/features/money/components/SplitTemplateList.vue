<script setup lang="ts">
  ...
  type SplitTemplate = PresetTemplate | CustomTemplate;

  const splitStore = useFamilySplitStore();
  const showConfigurator = ref(false);
  const selectedTemplate = ref<SplitTemplate | null>(null);
  ...
  // 应用模板
  function applyTemplate(template: SplitTemplate) {
...
<template>
  <div class="flex flex-col gap-8">
    <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
      <h3 class="m-0 text-2xl font-semibold text-gray-900 dark:text-white">分摆模板</h3>
    ...
    <section class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm">
      <h4 class="m-0 mb-2 text-lg font-semibold text-gray-900 dark:text-white">预设模板</h4>
    ...
    <!-- 自定义模板 -->
    <section class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm">
      <h4 class="m-0 mb-2 text-lg font-semibold text-gray-900 dark:text-white">自定义模板</h4>
      <p class="m-0 mb-6 text-gray-600 dark:text-gray-400 text-sm">
      ...
      <div
        ...
        class="grid grid-cols-1 md:grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-6"
      >
        <div
        ...
        >
          <div class="flex-1 flex flex-col gap-2">
            ...
            <p
              ...
              class="m-0 text-gray-600 dark:text-gray-400 text-sm leading-relaxed"
            >
              {{ template.description }}
            </p>
            <div class="flex gap-4 mt-2">
              <span
                class="inline-block px-3 py-1 bg-gray-100 dark:bg-gray-600 rounded-xl text-xs font-medium text-gray-700 dark:text-gray-200"
              >
          ...
          <div class="flex gap-2 items-center">
            ...
            <Button variant="primary" class="flex-1" @click="applyTemplate(template)">应用</Button>
          </div>
        </div>
      </div>

      <div v-else class="flex flex-col items-center gap-2 py-12 text-center">
        <p class="m-0 text-base text-gray-600 dark:text-gray-400">暂无自定义模板</p>
        <span class="text-sm text-gray-400 dark:text-gray-500"
          >点击“新建模板”创建您的第一个自定义分摆规则</span
        >
      </div>
    </section>
