<script setup lang="ts">
  ...
  function getDayOfYearDescription(dayOfYear: number): string {
    ...
    if (dayOfYear > 59 && !isCurrentLeapYear && dayOfYear <= 365) {
      // 平年��况下，显示当前年份的实际日期
  ...
  function removeMember(index: number) {
    if (memberList.value[index]?.isPrimary) return;
  ...
  function buildLedgerForm(source: FamilyLedger | null): FamilyLedger {
    ...
    return {
      ...
      settlementDay:
        source.settlementDay && source.settlementDay > 0
          ? source.settlementDay
          : defaultLedger.settlementDay,
    };
  }
    ...
    <form @submit.prevent="saveLedger">
      ...
      <FormSection title="基本信息">
        <FormRow label="账本名称" required>
          <Input v-model="form.name" type="text" placeholder="请输入账本名称" />
        </FormRow>
      ...
      <FormSection title="结算设置">
        <FormRow label="结算周期" optional>
          <Select v-model="form.settlementCycle" :options="settlementCycleOptions" />
        </FormRow>
        ...
        <div class="mb-3">
          <Checkbox v-model="form.autoSettlement" label="启用自动结算" />
        </div>
      </FormSection>
      ...
      <div class="flex flex-col gap-3">
        ...
        <div
          ...
          class="max-h-40 overflow-y-auto flex flex-col gap-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
        >
          <div
        ...
        <div
          ...
          class="mt-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 max-h-60 overflow-hidden flex flex-col shadow-md"
        >
          <div
            class="px-3 py-3 border-b border-gray-300 dark:border-gray-700 flex items-center justify-between font-medium text-sm text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-900/50"
          >
            <span>选择账户</span>
            <button
              type="button"
          ...
          <div
            class="overflow-y-auto p-2 flex flex-col gap-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
          >
            <label
