      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        >
          <div class="flex items-center gap-4 mb-4">
            ...
            >
              <component :is="role.icon" class="w-7 h-7 opacity-100" :style="{ color: role.color }" />
            </div>
          ...
          <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
            <div class="flex justify-between items-center mb-3">
              ...
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ role.permissions.length }} 项</span>
            </div>
    ...
    <Card v-if="isOwner" padding="lg" class="border-2 border-dashed">
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <Button variant="primary" @click="saveCustomRole">
            保存自定义角色
          </Button>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-4">
        <div class="flex items-start gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg">
          ...
          <div class="text-sm leading-relaxed text-gray-700 dark:text-gray-300">
            <strong class="text-gray-900 dark:text-white">所有者</strong>：拥有所有权限，包括删除账本和转移所有权
          </div>
        </div>
        <div class="flex items-start gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg">
          <LucideUserCog class="w-6 h-6 shrink-0 text-blue-600 dark:text-blue-500" />
          <div class="text-sm leading-relaxed text-gray-700 dark:text-gray-300">
            <strong class="text-gray-900 dark:text-white">管理员</strong>：可以管理交易和成员，但不能删除账本
          </div>
        </div>
        <div class="flex items-start gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg">
          <LucideUser class="w-6 h-6 shrink-0 text-green-600 dark:text-green-500" />
          <div class="text-sm leading-relaxed text-gray-700 dark:text-gray-300">
            <strong class="text-gray-900 dark:text-white">成员</strong>：可以添加和编辑自己的交易，查看统计信息
          </div>
        </div>
        <div class="flex items-start gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg">
          <LucideUser class="w-6 h-6 shrink-0 text-gray-600 dark:text-gray-500" />
          <div class="text-sm leading-relaxed text-gray-700 dark:text-gray-300">
            <strong class="text-gray-900 dark:text-white">查看者</strong>：只能查看交易和统计，不能进行任何修改
          </div>
        </div>
      </div>
    </div>
