<script setup lang="ts">
  ...
  interface SplitConfig {
    ...
    calculatedSplit?: SplitMemberResult[];
  }
  ...
  // 处理尾数
  function handleRemainder(splits: SplitMemberResult[], totalAmount: number) {
...
<template>
  <div class="fixed inset-0 z-[1000] flex items-center justify-center">
    ...
    <div
      class="relative w-[90%] max-w-[800px] max-h-[90vh] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
    >
      <!-- Header -->
      <div
        class="flex justify-between items-center px-6 md:px-8 py-6 border-b border-gray-200 dark:border-gray-700"
      >
        <h3 class="m-0 text-xl font-semibold text-gray-900 dark:text-white">分摆规则配置</h3>
        <button
      ...
      <div class="flex justify-between p-8 bg-gray-50 dark:bg-gray-700">
        <div v-for="step in 4" :key="step" class="flex flex-col items-center gap-2 flex-1 relative">
          <!-- 连接线 -->
          <div
            v-if="step < 4"
            class="absolute top-5 left-1/2 right-[-50%] h-0.5 z-0"
            :class="[
          ...
          <!-- 步骤圆圈 -->
          <div
            class="w-10 h-10 rounded-full flex items-center justify-center font-semibold z-10 transition-all duration-300"
            :class="[
      ...
      <div class="flex-1 overflow-y-auto p-8">
        ...
        <div v-if="currentStep === 1" class="flex flex-col gap-6">
          <h4 class="m-0 text-lg font-semibold text-gray-900 dark:text-white">选择分摊类型</h4>
          <p class="m-0 text-gray-600 dark:text-gray-400 text-sm">选择适合您需求的分摊方式</p>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label
              v-for="type in splitTypes"
              :key="type.value"
              class="p-6 border-2 rounded-xl cursor-pointer transition-all duration-300 text-center"
              :class="[
            ...
            >
              <input v-model="config.splitType" type="radio" :value="type.value" hidden />
              <div
              ...
              <h5 class="m-0 mb-2 text-base font-semibold text-gray-900 dark:text-white">
                {{ type.label }}
              </h5>
        ...
        <div v-if="currentStep === 2" class="flex flex-col gap-6">
          <div class="flex justify-between items-start">
            <div>
              <h4 class="m-0 text-lg font-semibold text-gray-900 dark:text-white">选择参与成员</h4>
          ...
          <div class="grid grid-cols-2 md:grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-4">
            ...
            >
              ...
              />
              <div
                class="flex flex-col items-center gap-3 p-4 border-2 rounded-xl transition-all"
                :class="[
          ...
          <div
            v-if="config.selectedMembers.length > 0"
            class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg text-center text-sm text-gray-600 dark:text-gray-400"
          >
        ...
        <div v-if="currentStep === 3" class="flex flex-col gap-6">
          <h4 class="m-0 text-lg font-semibold text-gray-900 dark:text-white">配置分摊参数</h4>
          ...
          <div v-if="config.splitType === 'EQUAL'">
            <div
              class="flex items-center gap-4 p-6 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-600 dark:border-blue-500 rounded-xl"
            >
              <LucideEqual class="w-8 h-8 text-blue-600 dark:text-blue-400 shrink-0" />
          ...
          <div v-if="config.splitType === 'PERCENTAGE'" class="flex flex-col gap-4">
            ...
            >
              总计:
              {{ config.selectedMembers.reduce((sum, id) => sum + (config.splitParams[id]?.percentage || 0), 0).toFixed(1) }}
          ...
          <div v-if="config.splitType === 'FIXED_AMOUNT'" class="flex flex-col gap-4">
            ...
            >
              总计: ¥
              {{ config.selectedMembers.reduce((sum, id) => sum + (config.splitParams[id]?.amount || 0), 0).toFixed(2) }}
              / ¥{{ amount.toFixed(2) }}
              <span v-if="isValid" class="ml-2 text-emerald-600 dark:text-emerald-400">✓</span>
          ...
          <div v-if="config.splitType === 'WEIGHTED'" class="flex flex-col gap-4">
            ...
            <div
              class="p-4 bg-emerald-100 dark:bg-emerald-900/20 border border-emerald-600 dark:border-emerald-500 rounded-lg text-center font-medium text-emerald-700 dark:text-emerald-300"
            >
              总权重:
              {{ config.selectedMembers.reduce((sum, id) => sum + (config.splitParams[id]?.weight || 0), 0) }}
            </div>
          </div>
        </div>
        ...
        <div v-if="currentStep === 4" class="flex flex-col gap-6">
          <h4 class="m-0 text-lg font-semibold text-gray-900 dark:text-white">预览分摊结果</h4>
          <p class="m-0 text-gray-600 dark:text-gray-400 text-sm">
            确认分摊结果无误后，可以保存或应用
          </p>
          ...
          <div class="flex flex-col gap-6">
            <div
              class="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 bg-gray-50 dark:bg-gray-700 rounded-xl"
            >
              ...
              >
                ...
                <div class="flex items-center gap-4">
                  ...
                  <strong class="text-lg font-semibold text-blue-600 dark:text-blue-400"
                    >{{ formatAmount(result.amount) }}</strong
                  >
                </div>
              </div>
            </div>
      ...
      <!-- Footer -->
      <div
        class="flex gap-4 px-6 md:px-8 py-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700"
      >
        ...
        <Button v-if="currentStep < 4" variant="primary" :disabled="!isValid" @click="nextStep">
          下一步
          <LucideChevronRight class="w-4 h-4" />
        </Button>

        <template v-if="currentStep === 4">
          <Button v-if="config.templateName" variant="secondary" @click="saveAsTemplate">
