...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div v-else-if="filteredRecords.length > 0" class="flex flex-col gap-4">
      ...
      >
        <div
          class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 pb-3 border-b border-gray-200 dark:border-gray-700"
        >
          <div class="flex items-center gap-3">
            <h4 class="text-base font-semibold text-gray-900 dark:text-white">交易分摊</h4>
          ...
          <div v-if="getMemberSplitDetail(record)" class="flex items-center gap-2">
            ...
            />
            <span
              class="text-sm font-medium px-3 py-1 rounded-xl"
              :class="[
        ...
        <div class="flex flex-col gap-3">
          ...
          <div
            v-if="getMemberSplitDetail(record)"
            class="flex justify-between items-center text-sm p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg font-medium"
          >
        ...
        <div
          v-if="getMemberSplitDetail(record)?.isPaid"
          class="flex items-center gap-2 mt-4 p-3 bg-green-100 dark:bg-green-900/30 rounded-lg text-green-800 dark:text-green-300 text-sm font-medium"
        >
          <LucideCheck :size="16" />
          <span>已支付</span>
        </div>
    ...
    <div v-else class="flex flex-col items-center gap-3 py-12 text-center">
      ...
      <p class="text-base text-gray-600 dark:text-gray-400">暂无分摊记录</p>
      <span class="text-sm text-gray-400 dark:text-gray-500"
        >参与家庭账本的分摊后，记录将显示在这里</span
      >
    </div>
