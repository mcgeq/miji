  ...
  >
    <div class="flex flex-col gap-6">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl border-2 border-gray-200 dark:border-gray-600">
          <LucideUsers class="w-6 h-6 flex-shrink-0 text-gray-600 dark:text-gray-300" />
          <div class="flex flex-col gap-1">
            <label class="text-xs text-gray-600 dark:text-gray-400">参与人数</label>
            <strong class="text-base font-semibold text-gray-900 dark:text-white">{{ statistics.total }} 人</strong>
          </div>
        </div>

        <div class="flex items-center gap-3 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border-2 border-emerald-600 dark:border-emerald-500">
          <LucideCheck class="w-6 h-6 flex-shrink-0 text-emerald-600 dark:text-emerald-400" />
          <div class="flex flex-col gap-1">
            <label class="text-xs text-gray-600 dark:text-gray-400">已支付</label>
            <strong class="text-base font-semibold text-gray-900 dark:text-white">{{ statistics.paid }} 人</strong>
          </div>
        </div>

        <div class="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border-2 border-amber-500 dark:border-amber-400">
          <LucideClock class="w-6 h-6 flex-shrink-0 text-amber-500 dark:text-amber-400" />
          <div class="flex flex-col gap-1">
            <label class="text-xs text-gray-600 dark:text-gray-400">待支付</label>
            <strong class="text-base font-semibold text-gray-900 dark:text-white">{{ statistics.unpaid }} 人</strong>
          </div>
        </div>

        <div class="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border-2 border-blue-500 dark:border-blue-400">
          <LucideCoins class="w-6 h-6 flex-shrink-0 text-blue-500 dark:text-blue-400" />
          <div class="flex flex-col gap-1">
            <label class="text-xs text-gray-600 dark:text-gray-400">总金额</label>
            <strong class="text-base font-semibold text-gray-900 dark:text-white">{{ formatAmount(statistics.totalAmount) }}</strong>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
        <div class="flex justify-between items-center text-sm font-medium">
          ...
          <span class="text-blue-600 dark:text-blue-400 font-semibold">{{ statistics.paidPercentage.toFixed(0) }}%</span>
        </div>
        ...
        <div class="flex justify-between text-xs">
          <span class="text-emerald-600 dark:text-emerald-400">已支付: {{ formatAmount(statistics.paidAmount) }}</span>
          <span class="text-amber-500 dark:text-amber-400">待支付: {{ formatAmount(statistics.unpaidAmount) }}</span>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
          <div
            ...
            class="p-4 rounded-xl border-2 transition-all duration-200" :class="[
          ...
          >
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-blue-500 dark:bg-blue-600 text-white flex items-center justify-center font-semibold text-base">
                  {{ detail.memberName.charAt(0) }}
                </div>
                <div class="flex flex-col gap-1">
                  <span class="text-sm font-medium text-gray-900 dark:text-white">{{ detail.memberName }}</span>
              ...
              <div class="flex items-center gap-4">
                <strong class="text-lg font-semibold text-blue-600 dark:text-blue-400">{{ formatAmount(detail.amount) }}</strong>
                <button
                  class="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium cursor-pointer transition-all duration-200" :class="[
                    detail.isPaid
            ...
            <div v-if="detail.isPaid && detail.paidAt" class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
              <span class="text-xs text-gray-500 dark:text-gray-400">支付时间: {{ formatDate(detail.paidAt) }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
        <div class="flex justify-between items-center text-sm">
          ...
          <strong class="font-semibold text-gray-900 dark:text-white">{{ formatAmount(transactionAmount) }}</strong>
        </div>
