...
<template>
  <div class="mt-4">
    ...
    <EmptyState v-else-if="transactions.length === 0" icon="📝" :message="emptyText" />

    <!-- Table 布局 -->
    <div
      v-else-if="layout === 'table'"
      class="overflow-x-auto rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm"
    >
      <table class="w-full border-separate border-spacing-0">
        <thead
          class="bg-gradient-to-b from-white to-gray-50 dark:from-gray-800 dark:to-gray-700 border-b-2 border-gray-200 dark:border-gray-600"
        >
          <tr>
            <th
              v-if="showColumn('date')"
              :class="compact ? 'px-3 py-2.5 text-[10px]' : 'px-4 py-3.5 text-[11px]'"
              class="text-left font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap border-b-2 border-gray-200 dark:border-gray-600"
            >
              日期
            </th>
            <th
            ...
            <th
              v-if="showActions"
              :class="compact ? 'px-3 py-2.5 text-[10px]' : 'px-4 py-3.5 text-[11px]'"
              class="text-center font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap border-b-2 border-gray-200 dark:border-gray-600"
            >
              操作
            </th>
          </tr>
        ...
        <tbody>
          ...
          >
            ...
            <td
              v-if="showColumn('date')"
              :class="compact ? 'px-3 py-2.5 text-xs' : 'px-4 py-3.5 text-[13px]'"
              class="text-gray-500 dark:text-gray-400 font-medium tabular-nums border-b border-gray-100 dark:border-gray-700"
            >
            ...
            >
              <Badge
                :variant="getTransactionTypeBadgeVariant(transaction.transactionType)"
                size="sm"
              >
            ...
            <td
              v-if="showColumn('category')"
              :class="compact ? 'px-3 py-2.5' : 'px-4 py-3.5'"
              class="min-w-[100px] border-b border-gray-100 dark:border-gray-700"
            >
              <div class="flex flex-col gap-0.5">
                <span class="font-medium text-gray-900 dark:text-white text-[13px]"
                  >{{ getCategoryName(transaction.category) }}</span
                >
                <span
            ...
            <td
              v-if="showColumn('account')"
              :class="compact ? 'px-3 py-2.5 text-xs' : 'px-4 py-3.5 text-[13px]'"
              class="font-medium text-gray-900 dark:text-white border-b border-gray-100 dark:border-gray-700"
            >
            ...
            <td
              v-if="showColumn('description')"
              :class="compact ? 'px-3 py-2.5 text-xs' : 'px-4 py-3.5 text-[13px]'"
              class="text-gray-500 dark:text-gray-400 max-w-[200px] md:max-w-[120px] overflow-hidden text-ellipsis whitespace-nowrap border-b border-gray-100 dark:border-gray-700"
            >
            ...
            <td
              v-if="showActions"
              :class="compact ? 'px-3 py-2.5' : 'px-4 py-3.5'"
              class="text-center border-b border-gray-100 dark:border-gray-700"
            >
    ...
    <div v-else-if="layout === 'card'" class="flex flex-col gap-3 py-2">
      ...
      >
        <!-- 类型列 -->
        <div
          class="text-sm px-4 py-3 flex justify-between items-center border-b border-gray-200/50 dark:border-gray-600/50 transition-colors gap-4 lg:border-b-0 lg:py-3 lg:px-2 lg:justify-end hover:bg-gray-100/50 dark:hover:bg-gray-700/50 lg:hover:bg-transparent"
        >
          <span
            class="inline-block text-gray-500 dark:text-gray-400 font-bold text-xs uppercase tracking-wider min-w-[80px] shrink-0 opacity-80 lg:hidden"
            >类型</span
          >
          <div class="flex items-center justify-end gap-2">
            <component :is="getTransactionTypeIcon(transaction.transactionType)" class="w-4 h-4" />
            <span class="font-medium"
              >{{ getTransactionTypeName(transaction.transactionType) }}</span
            >
          </div>
        </div>
        ...
        >
          ...
          >
          <div
            class="font-bold text-lg text-right tabular-nums -tracking-tight"
            :class="[
        ...
        <!-- 账户列 -->
        <div
          class="text-sm px-4 py-3 flex justify-between items-center border-b border-gray-200/50 dark:border-gray-600/50 transition-colors gap-4 lg:border-b-0 lg:py-3 lg:px-2 hover:bg-gray-100/50 dark:hover:bg-gray-700/50 lg:hover:bg-transparent"
        >
          <span
            class="inline-block text-gray-500 dark:text-gray-400 font-bold text-xs uppercase tracking-wider min-w-[80px] shrink-0 opacity-80 lg:hidden"
            >账户</span
          >
          <div class="flex-1 text-right min-w-0 wrap-break-words lg:text-right">
            <div class="font-medium text-gray-900 dark:text-white">
              {{ transaction.account.name }}
            </div>
            <div
              v-if="transaction.description"
              class="mt-1 text-gray-500 dark:text-gray-400 text-xs"
            >
              {{ transaction.description }}
            </div>
          </div>
        ...
        <div
          v-if="showActions"
          class="text-sm px-2 py-2 pb-3 flex justify-center items-center gap-1 shrink-0 lg:py-3 lg:px-1"
        >
          <button
