<script setup lang="ts">
  ...
  const emit = defineEmits<{
    'update:splitConfig': [config: EmittedSplitConfig];
  }>();

  const enableSplit = ref(false);
  const selectedTemplate = ref<QuickTemplate | null>(null);
  ...
  const splitPreview = computed(() => {
    ...
    const getMemberName = (memberId: string): string => {
      ...
      const member = props.availableMembers?.find(m => m.serialNum === memberId);
  ...
  function getDefaultPercentage(memberCount: number): number {
    return memberCount > 0 ? Number((100 / memberCount).toFixed(2)) : 0;
  }

  function getDefaultWeight(memberCount: number): number {
    return memberCount > 0 ? Number((10 / memberCount).toFixed(2)) : 0;
  }

  function initializeMemberParam(
    memberId: string,
    ...
    const param = splitConfig.splitParams[memberId];
    if (param.percentage === undefined) param.percentage = defaultPercentage;
    if (param.amount === undefined) param.amount = 0;
    if (param.weight === undefined) param.weight = defaultWeight;
  }

  function initializeSplitParams(): void {
    const memberCount = splitConfig.selectedMembers.length;
    const defaultPercentage = getDefaultPercentage(memberCount);
    const defaultWeight = getDefaultWeight(memberCount);

    splitConfig.selectedMembers.forEach(memberId => {
      initializeMemberParam(memberId, defaultPercentage, defaultWeight);
      ...
      if (splitConfig.splitType === 'PERCENTAGE') {
        ...
        if (
          splitConfig.splitParams[memberId].weight === undefined ||
          splitConfig.splitParams[memberId].weight === 0
        ) {
  ...
  function handleAmountInput(changedMemberId: string, newValue: number) {
    ...
    if (splitConfig.selectedMembers.length === 2) {
      ...
      const remaining = props.transactionAmount - finalValue;
      if (!splitConfig.splitParams[otherMemberId]) {
        splitConfig.splitParams[otherMemberId] = {};
      }
    ...
    config => {
      if (config?.enabled) {
        ...
        splitConfig.splitType =
          (config.splitType && splitTypeMap[config.splitType]) ||
          (config.splitType as SplitRuleType) ||
          'EQUAL';
        ...
        if (config.members && config.members.length > 0) {
          ...
          config.members.forEach(member => {
            ...
            if (member.percentage !== undefined && member.percentage !== null) {
              splitConfig.splitParams[member.memberSerialNum].percentage = Number(
                member.percentage,
              );
            }
  ...
  // 监听配置变化，通知父组件
  watch(
    [enableSplit, splitConfig, splitPreview],
    () => {
...
<template>
  <div
    class="flex flex-col gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700"
  >
    <!-- 分摊开关 -->
    ...
    <div
      v-if="enableSplit"
      class="flex flex-col gap-4 pt-4 border-t border-gray-200 dark:border-gray-700"
    >
      <!-- 快速模板选择 -->
      <div class="flex flex-col gap-3">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
          <button
            ...
            :key="template.id"
            type="button"
            class="flex flex-col items-center gap-1 px-2 py-2 border-2 rounded-md cursor-pointer transition-all text-xs disabled:cursor-not-allowed disabled:opacity-60"
            :class="[
      ...
      <div v-if="splitConfig.splitType !== 'EQUAL'" class="flex flex-col gap-3">
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            <span class="text-sm font-medium text-gray-700 dark:text-gray-200 min-w-[80px]"
              >{{ props.availableMembers?.find((m: any) => m.serialNum === memberId)?.name || 'Unknown' }}</span
            >
            <div class="flex items-center gap-2 flex-1 justify-end">
              ...
              <span
                v-if="splitConfig.splitType === 'PERCENTAGE'"
                class="text-sm text-gray-500 dark:text-gray-400 font-medium min-w-[20px]"
                >%</span
              >
              <input
              ...
              <span
                v-if="splitConfig.splitType === 'WEIGHTED'"
                class="text-xs text-blue-600 dark:text-blue-400 font-semibold px-2 py-1 bg-blue-600/10 dark:bg-blue-500/20 rounded min-w-[50px] text-center"
              >
      ...
      <!-- 分摊预览 -->
      <div
        v-if="splitPreview.length > 0"
        class="flex flex-col gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600"
      >
        <div class="flex justify-between items-center">
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">分摊预览</label>
          <span
            class="px-3 py-1 bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-xl text-xs font-medium"
          >
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            ...
            <strong class="text-sm font-semibold text-gray-700 dark:text-gray-200"
              >{{ formatAmount(item.amount) }}</strong
            >
          </div>
        </div>

        <div
          class="flex justify-between items-center pt-3 border-t-2 border-gray-200 dark:border-gray-600 text-sm font-semibold"
        >
          <span class="text-gray-900 dark:text-white">总计</span>
          <strong class="text-base font-semibold text-gray-900 dark:text-white"
            >{{ formatAmount(splitPreview.reduce((sum, item) => sum + item.amount, 0)) }}</strong
          >
        </div>
      </div>
    </div>
