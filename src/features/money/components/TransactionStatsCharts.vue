<script setup lang="ts">
  ...
  // ECharts tooltip formatter types
...
<template>
  <div class="mb-8 md:mb-8 w-full">
    <div class="grid grid-cols-1 gap-6 md:gap-6 w-full">
      <!-- 收支趋势图 -->
      <div
        class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 md:p-6 sm:p-4 shadow-sm w-full overflow-hidden"
      >
        <div class="flex justify-between items-center mb-4 flex-wrap gap-2">
          <div>
            <h3
              class="text-lg md:text-lg sm:text-base font-semibold text-gray-900 dark:text-white mb-1 wrap-break-words"
            >
              收支趋势
            </h3>
            <div class="text-sm text-gray-500 dark:text-gray-400 wrap-break-words">
              {{ timeDimension === 'week' ? '周度' : timeDimension === 'year' ? '年度' : '月度' }}
              趋势分析
            </div>
          </div>
        </div>
        ...
        <div class="min-h-[400px] md:min-h-[300px] sm:min-h-[250px] w-full overflow-hidden">
          <div
            v-if="loading"
            class="flex flex-col items-center justify-center h-[400px] md:h-[300px] sm:h-[250px] gap-4"
          >
            <Spinner size="lg" />
          ...
          <div
            v-else-if="currentTrends.length === 0"
            class="flex flex-col items-center justify-center h-[400px] md:h-[300px] sm:h-[250px] gap-4"
          >
