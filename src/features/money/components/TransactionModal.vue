<script setup lang="ts">
  ...
  const form = ref<Transaction>(
    initializeFormData(props.transaction || null, props.type, props.accounts),
  );
  ...
  // 支付方式管理
  const { availablePaymentMethods, isPaymentMethodEditable: baseIsPaymentMethodEditable } =
    usePaymentMethods(
      computed(() => props.accounts),
  ...
  function handleSplitConfigUpdate(config: SplitConfigUpdate) {
  ...
  async function loadInstallmentPlanDetails(planSerialNum: string) {
    ...
    if (installmentManager.planDetails.value) {
      processInstallmentPlanResponse(installmentManager.planDetails.value);
    }
  }
  ...
  function processInstallmentPlanResponse(response: DeepReadonly<InstallmentPlanResponse> | null) {
    if (response?.details) {
      // 更新表单中的分期相关字段
  ...
  // 处理账户变化：智能推荐账本（仅创建模式）
  async function handleAccountChangeForLedgers(
  ...
  function handleAccountChangeForPayment(accountId: string | null) {
    ...
    // 自动设置支付方式
    form.value.paymentMethod = getDefaultPaymentMethod(selectedAccount, form.value.transactionType);
    ...
    if (!form.value.isInstallment) {
      return [
        ...
        },
        {
          ...
          label: t('financial.transaction.statusOptions.completed'),
        },
        {
          value: TransactionStatusSchema.enum.Reversed,
          label: t('financial.transaction.statusOptions.reversed'),
        },
      ];
    }
  ...
  watch(
    [() => form.value.amount, () => form.value.totalPeriods, () => form.value.firstDueDate],
    () => {
      if (form.value.isInstallment) {
        calculateInstallmentFromBackend();
      }
  ...
  function emitTransfer(amount: number) {
    const fromTransaction: TransferCreate = {
      ...
      date: DateUtils.formatDateToBackend(
        typeof form.value.date === 'string' ? new Date(form.value.date) : form.value.date,
      ),
      description: form.value.description,
    };

    if (props.transaction?.relatedTransactionSerialNum) {
  ...
  watch(
    ...
    async transaction => {
      if (transaction) {
        form.value = initializeFormData(transaction, props.type, props.accounts);
  ...
  >
    <form @submit.prevent="handleSubmit">
      ...
      <FormRow v-if="props.transaction" :label="t('financial.transaction.transType')" required>
        <div
          class="px-4 py-2.5 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white font-medium"
        >
      ...
      <FormRow v-if="!isReadonlyMode || selectedLedgers.length > 0" label="关联账本" optional>
        <div class="flex items-center gap-2 w-full">
          <div
            class="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 min-h-[42px]"
          >
            <div
            ...
            <div v-else class="flex items-center gap-2 flex-wrap">
              <span
                class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-md text-sm font-medium"
              >
      ...
      <div v-if="!isReadonlyMode && showLedgerSelector" class="mb-4 -mt-2">
        ...
        >
          ...
          <div class="flex flex-col gap-2 max-h-60 overflow-y-auto">
            ...
            >
              <Checkbox v-model="selectedLedgers" :value="ledger.serialNum" />
              <div class="flex-1 flex items-center justify-between">
                <span class="text-sm font-medium text-gray-900 dark:text-white"
                  >{{ ledger.name }}</span
                >
                <span
                  class="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded"
                  >{{ ledger.ledgerType }}</span
                >
              </div>
            </label>
          </div>
      ...
      <!-- 分摆成员 -->
      <FormRow
        v-if="selectedLedgers.length > 0 && (!isReadonlyMode || selectedMembers.length > 0)"
        label="分摆成员"
        optional
      >
        <div class="flex flex-col gap-1.5 w-full">
          <div class="flex items-center gap-2">
            ...
            >
              ...
              <div v-else class="flex items-center gap-2 flex-wrap">
                <span
                  class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-md text-sm font-medium"
                >
          ...
          <div
            v-if="!isReadonlyMode && selectedMembers.length === 0"
      ...
      <!-- 成员选择下拉 -->
      <div
        v-if="!isReadonlyMode && selectedLedgers.length > 0 && showMemberSelector"
        class="mb-4 -mt-2"
      >
        <div
          class="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900"
        >
          ...
          <div class="flex flex-col gap-2 max-h-60 overflow-y-auto">
            <label
              ...
              class="flex items-center gap-3 p-3 rounded-lg hover:bg-white dark:hover:bg-gray-800 border border-transparent hover:border-gray-200 dark:hover:border-gray-700 cursor-pointer transition-all"
            >
              <Checkbox v-model="selectedMembers" :value="member.serialNum" />
              <span class="text-sm font-medium text-gray-900 dark:text-white"
                >{{ member.name }}</span
              >
            </label>
      ...
      <!-- 分期选项 -->
      <div
        v-if="form.transactionType === 'Expense' && !isCurrentTransactionInstallment"
      ...
      <div v-if="form.isInstallment" class="installment-section">
        ...
        <!-- 分期计划详情 -->
        <div
          v-if="installmentDetails"
          class="mt-4 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900"
        >
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-2">
                    <span class="text-sm font-medium text-gray-900 dark:text-white"
                      >第 {{ detail.period || (index + 1) }}期</span
                    >
                    <span
                      v-if="detail.status"
                ...
                <div class="text-right">
                  ...
                  <div v-if="detail.status === 'PAID'" class="mt-2 flex flex-col gap-1">
                    <div
                      class="flex items-center justify-end gap-1.5 text-xs text-green-600 dark:text-green-400"
                    >
                      <span>✓</span>
                      <span>入账:</span>
                  ...
                  <div v-else-if="detail.status === 'PENDING'" class="mt-2">
                    <span
                      class="inline-flex items-center gap-1 px-2 py-1 text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded"
                    >
                      ⏳ 待入账
                    </span>
                  </div>
                  <div v-else-if="detail.status === 'OVERDUE'" class="mt-2">
                    <span
                      class="inline-flex items-center gap-1 px-2 py-1 text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded"
                    >
                      ⚠️ 已逾期
                    </span>
                  </div>
              ...
              <div class="flex gap-4">
                <div class="flex items-center gap-1.5 text-sm">
                  ...
                  <span class="font-medium text-green-600 dark:text-green-400"
                    >{{ paidPeriodsCount }}期</span
                  >
                </div>
                <div class="flex items-center gap-1.5 text-sm">
                  <span class="text-gray-600 dark:text-gray-400">待入账:</span>
                  <span class="font-medium text-blue-600 dark:text-blue-400"
                    >{{ pendingPeriodsCount }}期</span
                  >
                </div>
                <div class="flex items-center gap-1.5 text-sm">
                  <span class="text-gray-600 dark:text-gray-400">总期数:</span>
                  <span class="font-medium text-gray-900 dark:text-white"
                    >{{ totalPeriodsCount }}期</span
                  >
                </div>
              </div>
            </div>
            <div class="flex items-center justify-between">
              <strong class="text-base text-gray-900 dark:text-white"
                >{{ t('financial.transaction.totalAmount') }}: ¥{{ safeToFixed(form.amount) }}</strong
              >
              <button
                v-if="hasMorePeriods"
                type="button"
