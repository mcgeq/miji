<script setup lang="ts">
  ...
  // 平衡成员列表
  const balanced = computed(() => memberBalances.value.filter(m => Math.abs(m.balance) < 0.01));

  // 获取成员相关的债务关系
  function getMemberDebts(memberSerialNum: string) {
    return debtRelations.value.filter(
      debt =>
        !debt.isSettled &&
...
<template>
  <div class="p-4 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
      <h3 class="text-xl font-semibold text-gray-900 dark:text-white">债务关系图</h3>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <Card padding="md" hoverable>
        <div class="flex items-center gap-3">
          <div
            class="p-2 rounded-md bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400"
          >
          ...
          <div class="flex-1">
            ...
            <div class="text-sm text-gray-500 dark:text-gray-400">债权人</div>
          </div>
        </div>
      </Card>
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1">
            ...
            <div class="text-sm text-gray-500 dark:text-gray-400">债务人</div>
          </div>
        </div>
      </Card>
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1">
            ...
            <div class="text-sm text-gray-500 dark:text-gray-400">已平衡</div>
          </div>
        </div>
      </Card>
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center gap-3">
          <div
            class="p-2 rounded-md bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400"
          >
            <LucideUsers :size="20" />
          </div>
          <div class="flex-1">
            ...
            <div class="text-sm text-gray-500 dark:text-gray-400">总成员</div>
          </div>
        </div>
      </Card>
    </div>
    ...
    <!-- 成员余额列表 -->
    <div class="mb-8">
      <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">成员余额</h4>

      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- 债权人 -->
        <Card v-if="creditors.length > 0" padding="none">
          <h5
            class="text-base font-semibold px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700 text-green-600 dark:text-green-400 m-0"
          >
            债权人
          </h5>
          <div class="p-2">
            ...
            >
              ...
              <div
                class="px-2 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400"
              >
        ...
        <Card v-if="debtors.length > 0" padding="none">
          <h5
            class="text-base font-semibold px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700 text-red-600 dark:text-red-400 m-0"
          >
            债务人
          </h5>
          <div class="p-2">
            ...
            >
              ...
              <div
                class="px-2 py-1 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400"
              >
                {{ getBalanceText(member.balance) }}
              </div>
            </div>
        ...
        <Card v-if="balanced.length > 0" padding="none">
          <h5
            class="text-base font-semibold px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 m-0"
          >
            已平衡
          </h5>
          <div class="p-2">
            ...
            >
              ...
              <div class="flex-1">
                ...
                <div class="text-sm font-semibold text-gray-500 dark:text-gray-400">¥0.00</div>
              </div>

              <div
                class="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"
              >
                {{ getBalanceText(member.balance) }}
              </div>
            </div>
          ...
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
              <span class="font-medium text-green-600 dark:text-green-400"
                >{{ getMemberInfo(debt.creditorMemberSerialNum).name }}</span
              >
              <LucideArrowRight :size="16" class="text-gray-400 dark:text-gray-600" />
              <span class="font-medium text-red-600 dark:text-red-400"
                >{{ getMemberInfo(debt.debtorMemberSerialNum).name }}</span
              >
            </div>
    ...
    <!-- 结算建议 -->
    <Card
      v-if="showSettlementSuggestions && settlementSuggestions.length > 0"
      padding="md"
      class="mb-8"
    >
      <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">结算建议</h4>
          ...
          <div class="flex items-center gap-4 mb-2">
            <div class="flex items-center gap-2 flex-1">
              ...
              <span class="font-medium text-gray-900 dark:text-white"
                >{{ suggestion.fromMemberName }}</span
              >
            </div>
            ...
            <div class="flex items-center gap-2 flex-1">
              ...
              <span class="font-medium text-gray-900 dark:text-white"
                >{{ suggestion.toMemberName }}</span
              >
            </div>
          </div>

          <div
            v-if="suggestion.relatedDebts.length > 0"
    ...
    <!-- 空状态 -->
    <div
      v-if="memberBalances.every(m => Math.abs(m.balance) < 0.01)"
      class="flex flex-col items-center justify-center py-12 text-center"
    >
      <LucideCheckCircle :size="64" class="text-green-500 dark:text-green-400 mb-4" />
      <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">所有账务已平衡</h3>
      <p class="text-gray-500 dark:text-gray-400">当前没有未结算的债务关系</p>
    </div>
  </div>
</template>
