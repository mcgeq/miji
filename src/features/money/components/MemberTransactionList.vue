...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div v-else-if="filteredTransactions.length > 0" class="flex flex-col gap-3">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <div
            class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 mb-2"
          >
          ...
          <div class="flex gap-2 sm:gap-4 flex-wrap text-sm text-gray-500 dark:text-gray-400">
            ...
            <span
              class="px-2 py-0.5 bg-gray-200 dark:bg-gray-700 rounded text-gray-700 dark:text-gray-300"
              >{{ transaction.category }}</span
            >
            <span
              v-if="transaction.splitMembers && transaction.splitMembers.length > 0"
              class="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 rounded font-medium"
            >
    ...
    <Card
      ...
      padding="md"
      class="bg-gray-50 dark:bg-gray-900/50 text-center"
    >
      <span class="text-sm text-gray-600 dark:text-gray-400"
        >共 {{ filteredTransactions.length }}笔交易</span
      >
    </Card>
  </div>
</template>
