<script setup lang="ts">
  ...
  const activeFiltersCount = computed(() => {
    ...
    if (localConfig.value.amountRange.min !== null || localConfig.value.amountRange.max !== null)
      count++;
    return count;
  });
</script>
...
<template>
  <div
    class="flex flex-col bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden"
  >
    <div
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <h3 class="m-0 text-lg font-semibold text-gray-900 dark:text-white">高级筛选</h3>
        <span
          v-if="activeFiltersCount > 0"
          class="inline-flex items-center justify-center min-w-[24px] h-6 px-2 bg-blue-600 dark:bg-blue-500 text-white rounded-xl text-xs font-semibold"
        >
          {{ activeFiltersCount }}
        </span>
      </div>
    ...
    <div class="px-6 py-4 flex flex-col gap-6 max-h-[500px] overflow-y-auto">
      ...
      <div class="flex flex-col gap-3">
        ...
        >
          <option v-for="option in splitTypeOptions" :key="option.value" :value="option.value">
            {{ option.label }}
          </option>
        </select>
      </div>
    ...
    <!-- 操作按钮 -->
    <div
      class="flex gap-4 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700"
    >
      <Button variant="secondary" class="flex-1" @click="resetFilter">
        <LucideX class="w-4 h-4" />
        重置
      </Button>
      <Button variant="primary" class="flex-1" @click="applyFilter">
