<script setup lang="ts">
  ...
  // 使用预算统计 composable
  const { state, filters, hasData, isHealthy, needsAttention, loadAllStats, resetFilters } =
    useBudgetStats();
  ...
  const chartKey = computed(
    () => `${chartType.value}-${trendChartType.value}-${categoryChartType.value}`,
  );
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="state.loading" class="flex flex-col items-center justify-center py-10">
      <Spinner size="lg" />
      <div class="text-sm text-gray-500 dark:text-gray-400 mt-4">加载统计数据中...</div>
    </div>
    ...
    >
      ...
      <div class="text-base font-medium mb-2">暂无统计数据</div>
      <div class="text-sm text-center max-w-md mb-4">
        <p class="my-1">系统中还没有预算数据，无法进行统计分析。</p>
        <p class="my-1">请先创建一些预算，然后再查看统计结果。</p>
      </div>
      <div
        class="bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 my-4 text-xs text-gray-600 dark:text-gray-400 text-left max-w-xs"
      >
        <p class="my-1">调试信息:</p>
        <p class="my-1">Loading: {{ state.loading }}</p>
        <p class="my-1">Has Data: {{ hasData }}</p>
        <p class="my-1">
          Overview:
          {{ state.overview ? `有数据 (${state.overview.budgetCount} 个预算)` : '无数据' }}
        </p>
        <p class="my-1">Error: {{ state.error || '无错误' }}</p>
      </div>
      <div class="flex gap-3 justify-center flex-wrap">
        <Button variant="primary" @click="goToBudgetList">去创建预算</Button>
        <Button variant="secondary" @click="loadAllStats">刷新数据</Button>
        <Button variant="success" @click="createTestData">创建测试数据</Button>
      </div>
    </div>
    ...
    <div v-if="state.overview" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center">
          ...
          <div class="flex-1">
            <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">总预算</div>
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center">
          ...
          <div class="flex-1">
            <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">已使用</div>
            <div class="text-xl font-bold text-gray-900 dark:text-white">
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center">
          ...
          <div class="flex-1">
            <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">剩余金额</div>
            <div class="text-xl font-bold text-gray-900 dark:text-white">
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center">
          ...
          <div class="flex-1">
            <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">完成率</div>
            <div class="text-xl font-bold" :class="statusColor">
              {{ formatPercentage(state.overview.completionRate) }}
            </div>
            <div class="text-xs mt-1" :class="statusColor">{{ statusText }}</div>
          </div>
        </div>
      </Card>
      ...
      <Card padding="md" hoverable>
        <div class="flex items-center">
          ...
          <div class="flex-1">
            <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">预算数量</div>
            <div class="text-xl font-bold text-gray-900 dark:text-white">
      ...
      <!-- 超预算数量 -->
      <Card
        v-if="state.overview.overBudgetCount > 0"
        padding="md"
        hoverable
      ...
      >
        <div class="flex items-center">
          ...
          <div class="flex-1">
            <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">超预算</div>
    ...
    <Card v-if="showFilters" padding="lg">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center gap-2">
          ...
          />
          <label class="text-sm text-gray-700 dark:text-gray-300">包含非激活预算</label>
        </div>
        ...
        <div class="flex gap-3 justify-end">
          <Button variant="secondary" @click="handleResetFilters">重置</Button>
          <Button variant="primary" @click="toggleFilters">应用</Button>
        </div>
      </div>
    </Card>
    ...
    <div
      v-if="state.error"
      class="flex items-center p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg"
    >
      <AlertCircle :size="20" class="text-red-600 dark:text-red-400 mr-3" />
      <div class="text-sm text-red-600 dark:text-red-400">{{ state.error }}</div>
    </div>

    <!-- 图表区域 -->
    <Card v-if="hasData" padding="lg">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white m-0">图表分析</h3>
      ...
      <div class="w-full h-96 relative">
        ...
        <div v-else-if="state.loading" class="flex flex-col items-center justify-center h-full">
          <Spinner size="lg" />
          <div class="text-sm text-gray-500 dark:text-gray-400 mt-4">加载中...</div>
        </div>
        <div
          v-else
