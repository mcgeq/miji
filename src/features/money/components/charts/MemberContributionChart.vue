<script setup lang="ts">
  ...
  // 计算总支付金额
  const totalPaid = computed(() => props.data.reduce((sum, member) => sum + member.totalPaid, 0));
...
<template>
  <div :style="{ height }">
    ...
    <div class="flex flex-col md:flex-row gap-6" :class="{ 'h-[calc(100%-3rem)]': title }">
      ...
      <div class="flex-[2] min-h-0">
        <div class="flex h-full">
          ...
          <div class="flex flex-col w-20 mr-4">
            <div
              class="text-xs text-gray-500 dark:text-gray-400 text-center mb-2 [writing-mode:vertical-rl]"
            >
              支付金额 (¥)
            </div>
            <div class="flex-1 flex flex-col justify-between relative">
              <div
                v-for="i in 5"
                :key="i"
                class="flex items-center justify-end relative after:content-[''] after:absolute after:right-[-8px] after:w-1 after:h-px after:bg-gray-300 dark:after:bg-gray-600"
              >
                <span class="text-xs text-gray-500 dark:text-gray-400 mr-2"
                  >{{ formatAmount(totalPaid * (5 - i + 1) / 5) }}</span
                >
              </div>
            </div>
          </div>
            ...
            <div class="flex items-end justify-around h-[calc(100%-2rem)] pt-4 px-2 pb-0">
              ...
              >
                ...
                <div class="relative w-full h-full flex items-end justify-center">
                  ...
                  >
                    <div
                      class="absolute bottom-full left-1/2 -translate-x-1/2 bg-black/80 text-white px-2 py-2 rounded text-xs whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10"
                    >
                ...
                <!-- X轴标签 -->
                <div
                  class="text-xs text-gray-500 dark:text-gray-400 text-center mt-2 max-w-full overflow-hidden text-ellipsis"
                >
                  {{ member.name }}
                </div>
              </div>
      ...
      <!-- 统计信息 -->
      <div
        class="flex-1 min-w-[200px] border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-700 pt-4 md:pt-0 md:pl-4"
      >
        <div class="mb-4">
          <h5 class="text-sm font-semibold text-gray-900 dark:text-white">详细统计</h5>
        </div>

        <div
          class="flex flex-col gap-4 max-h-[calc(100%-3rem)] overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
        >
          <div
            v-for="member in chartData"
            :key="member.name"
            ...
            <div class="flex flex-col gap-1">
              <div class="flex justify-between items-center">
                ...
                <span class="text-xs font-medium text-gray-900 dark:text-white"
                  >¥{{ formatAmount(member.totalPaid) }}</span
                >
              </div>
              <div class="flex justify-between items-center">
                <span class="text-xs text-gray-600 dark:text-gray-400">应分摆:</span>
                <span class="text-xs font-medium text-gray-900 dark:text-white"
                  >¥{{ formatAmount(member.totalOwed) }}</span
                >
              </div>
              <div class="flex justify-between items-center">
              ...
              <div class="flex justify-between items-center">
                ...
                <span class="text-xs font-medium text-gray-900 dark:text-white"
                  >{{ member.percentage.toFixed(1) }}%</span
                >
              </div>
            </div>
          </div>
