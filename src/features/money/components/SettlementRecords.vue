...
<template>
  <div class="p-4">
    ...
    <div class="flex items-center justify-between mb-6">
      <h3 class="text-xl font-semibold text-gray-900 dark:text-white">结算记录</h3>
      <Button variant="secondary" size="sm" :icon="RefreshCw" @click="fetchSettlementRecords">
        刷新
      </Button>
    </div>
    ...
    <div
      v-if="loading"
      class="flex items-center justify-center gap-2 p-8 text-gray-500 dark:text-gray-400"
    >
      <Spinner size="sm" />
      <span>加载中...</span>
    </div>
    ...
    <!-- 空状态 -->
    <div
      v-else-if="settlementRecords.length === 0"
      class="flex flex-col items-center justify-center p-12 text-center"
    >
      <FileText class="w-12 h-12 text-gray-400 dark:text-gray-600 mb-4" />
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">暂无结算记录</h3>
      <p class="text-gray-500 dark:text-gray-400">还没有进行过结算操作</p>
    </div>
    ...
    >
      <div class="flex flex-col gap-6">
        <div>
          <h4 class="text-base font-semibold text-gray-900 dark:text-white mb-3">基本信息</h4>
          <div class="grid gap-3">
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">结算日期:</span>
              <span class="text-sm font-medium text-gray-900 dark:text-white"
                >{{ formatDate(selectedRecord.settlementDate) }}</span
              >
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">结算周期:</span>
              <span class="text-sm font-medium text-gray-900 dark:text-white">
                {{ formatDate(selectedRecord.periodStart) }}-
                {{ formatDate(selectedRecord.periodEnd) }}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-gray-500 dark:text-gray-400">总金额:</span>
              <span class="text-sm font-medium text-gray-900 dark:text-white"
                >¥{{ formatAmount(selectedRecord.totalSettlementAmount) }}</span
              >
            </div>
          </div>
        </div>
        ...
        <div>
          <h4 class="text-base font-semibold text-gray-900 dark:text-white mb-3">转账明细</h4>
            ...
            >
              <div class="flex items-center gap-2">
                <span class="text-red-600 dark:text-red-400 font-medium"
                  >{{ getMemberName(settlement.fromMemberSerialNum) }}</span
                >
                <ArrowRight class="w-4 h-4 text-gray-400 dark:text-gray-600" />
                <span class="text-green-600 dark:text-green-400 font-medium"
                  >{{ getMemberName(settlement.toMemberSerialNum) }}</span
                >
              </div>
              <div class="font-semibold text-gray-900 dark:text-white">
        ...
        <div v-if="selectedRecord.notes">
          <h4 class="text-base font-semibold text-gray-900 dark:text-white mb-3">备注</h4>
