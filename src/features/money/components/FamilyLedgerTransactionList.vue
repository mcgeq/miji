<script setup lang="ts">
  ...
  async function loadTransactions() {
    ...
    try {
      // 1. 获取账本的交易关联
      associations.value = await MoneyDb.listFamilyLedgerTransactionsByLedger(
        props.ledgerSerialNum,
      );
  ...
  <div class="flex flex-col gap-4">
    <Card padding="md">
      <div class="flex items-center justify-between">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">账本交易记录</h3>
    ...
    <Card v-else-if="transactions.length === 0" padding="lg">
      <div class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p class="text-base font-medium text-gray-900 dark:text-white mb-2">暂无交易记录</p>
        <p class="text-sm text-gray-500 dark:text-gray-400">在此账本中创建交易后将显示在这里</p>
      </div>
    </Card>
      ...
      >
        <div class="flex items-center justify-between gap-4 mb-3">
          <div class="flex-1 min-w-0">
            <div
              class="font-medium text-gray-900 dark:text-white mb-2 overflow-hidden text-ellipsis whitespace-nowrap"
            >
            ...
            <div class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 flex-wrap">
              <span
                ...
                :class="getTransactionTypeBadgeClass(transaction.transactionType)"
              >
                {{ getTransactionTypeLabel(transaction.transactionType) }}
              </span>
              <span class="text-gray-500 dark:text-gray-500">
                {{ transaction.category }}
                <template v-if="transaction.subCategory">/ {{ transaction.subCategory }}</template>
              </span>
              <span class="text-gray-500 dark:text-gray-500">
          ...
          <div
            ...
            :class="getAmountColorClass(transaction.transactionType)"
          >
        ...
        <div
          ...
          class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 leading-relaxed"
        >
          {{ transaction.notes }}
        </div>
      </Card>
