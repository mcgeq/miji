<script setup lang="ts">
  ...
  // 响应式数据
  const pendingInstallments = ref<InstallmentPlan[]>([]);
  const showPaymentModal = ref(false);
  const selectedDetail = ref<InstallmentDetail | null>(null);
  ...
  const canConfirmPayment = computed(() => {
    return (
      ...
      paidAmount.value > 0 &&
      paidAmount.value <= (selectedDetail.value?.amount || 0)
    );
  });
  ...
  async function refreshData() {
    try {
      const response = await invokeCommand<InstallmentPlan[]>('installment_pending_list', {
  ...
  function payInstallment(detail: InstallmentDetail) {
...
<template>
  <div class="p-5 max-w-6xl mx-auto">
    ...
    <div>
      ...
      <div
        v-if="pendingInstallments.length === 0"
        class="text-center py-10 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 rounded-lg"
      >
            ...
            <div class="flex-1">
              ...
              <div class="space-y-1">
                <p class="text-sm text-gray-600 dark:text-gray-400">
                  {{ t('financial.installment.totalAmount') }}: ¥
                  {{ installment.totalAmount.toFixed(2) }}
                </p>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                  {{ t('financial.installment.remainingPeriods') }}:
                  {{ installment.remainingPeriods }}/{{ installment.totalPeriods }}
                </p>
              </div>
            </div>
            <div class="flex gap-2 flex-wrap">
              <button
                v-for="detail in installment.details.filter((d: InstallmentDetail) => d.status === 'PENDING')"
              ...
              >
                ...
                <div class="text-xs opacity-90">¥{{ detail.amount.toFixed(2) }}</div>
              </button>
            </div>
          </div>
        </Card>
      ...
      <div class="space-y-3">
        <div class="flex justify-between items-center">
          <label class="text-sm font-medium text-gray-900 dark:text-white">
            {{ t('financial.installment.paymentModal.period') }}
          </label>
          <span class="text-sm text-gray-600 dark:text-gray-400">{{ selectedDetail?.period }}</span>
        </div>
        <div class="flex justify-between items-center">
          <label class="text-sm font-medium text-gray-900 dark:text-white">
            {{ t('financial.installment.paymentModal.dueDate') }}
          </label>
          <span class="text-sm text-gray-600 dark:text-gray-400"
            >{{ selectedDetail?.dueDate }}</span
          >
        </div>
        <div class="flex justify-between items-center">
          <label class="text-sm font-medium text-gray-900 dark:text-white">
            {{ t('financial.installment.paymentModal.amount') }}
          </label>
          <span class="text-sm text-gray-600 dark:text-gray-400"
            >¥{{ selectedDetail?.amount.toFixed(2) }}</span
          >
        </div>
        <div class="flex justify-between items-center gap-4">
          <label class="text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">
            {{ t('financial.installment.paymentModal.paidAmount') }}
          </label>
          <Input
