<script setup lang="ts">
  ...
  // 初始化
  onMounted(async () => {
    await Promise.all([familyMemberStore.fetchMembers(), categoryStore.fetchCategories()]);
    ...
    <form class="flex flex-col gap-6" @submit.prevent="onSubmit">
      ...
      <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
        <div class="flex justify-between items-center mb-4">
          ...
          <Button type="button" variant="primary" size="sm" @click="handleAddAllocation">
            + 添加分配
          </Button>
        </div>
        ...
        <div
          ...
          class="p-4 bg-white dark:bg-gray-800 rounded-lg mb-4 flex flex-col gap-2"
        >
          <div class="flex items-center gap-2 text-sm">
            <span class="text-gray-600 dark:text-gray-400">已分配金额：</span>
            <span class="font-semibold text-blue-600 dark:text-blue-400"
              >¥{{ allocationsSummary.totalFixed.toFixed(2) }}</span
            >
            <span class="text-gray-500 dark:text-gray-500 text-xs"
              >(剩余: ¥{{ allocationsSummary.remainingFixed.toFixed(2) }})</span
            >
          </div>
          <div class="flex items-center gap-2 text-sm">
            <span class="text-gray-600 dark:text-gray-400">已分配百分比：</span>
            <span class="font-semibold text-blue-600 dark:text-blue-400"
              >{{ allocationsSummary.totalPercentage.toFixed(1) }}%</span
            >
            <span class="text-gray-500 dark:text-gray-500 text-xs"
              >(剩余: {{ allocationsSummary.remainingPercentage.toFixed(1) }}%)</span
            >
          </div>
        </div>
