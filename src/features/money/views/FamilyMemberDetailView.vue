...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      <div
        class="w-10 h-10 border-3 border-gray-300 dark:border-gray-700 border-t-blue-600 rounded-full animate-spin"
      />
    ...
    <template v-else-if="member">
      ...
      <header class="flex gap-4 mb-8">
        <button
          class="p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer transition-all hover:bg-gray-100 dark:hover:bg-gray-700"
          @click="goBack"
        >
          <LucideArrowLeft class="w-5 h-5" />
        </button>
        <div
          class="flex items-center gap-6 flex-1 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm"
        >
          <div
          ...
          <div>
            ...
            <p
              v-if="member.userSerialNum"
              class="mt-2 mb-0 text-gray-500 dark:text-gray-400 text-sm"
            >
      ...
      <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm flex items-center gap-4">
          <LucideWallet
            class="w-10 h-10 p-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-blue-600 dark:text-blue-400"
          />
          <div>
      ...
      <section class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        <div class="flex border-b border-gray-200 dark:border-gray-700">
          <button
            class="flex-1 px-4 py-4 bg-transparent border-none cursor-pointer text-base font-medium transition-all border-b-2"
            :class="[
    ...
    <div v-else class="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      <p class="text-gray-600 dark:text-gray-400">未找到成员信息</p>
      <button
