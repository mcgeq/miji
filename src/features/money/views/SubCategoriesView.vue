<script setup lang="ts">
  ...
  // 加载数据
  onMounted(async () => {
    await Promise.all([categoryStore.fetchCategories(), categoryStore.fetchSubCategories()]);
  });
  ...
  async function handleAddSubCategory(name: string, icon: string, categoryName: string) {
    try {
      ...
      const message = error instanceof Error ? error.message : '添加子分类失败';
      toast.error(message);
    }
  }
  ...
  <div class="space-y-4">
    ...
    <div v-else class="space-y-3">
      ...
      >
        ...
        <!-- 子分类列表 -->
        <div v-if="isCategoryExpanded(category.name)" class="p-4 bg-white dark:bg-gray-900">
          <div
          ...
          <div v-else class="text-center py-4 text-gray-400 text-sm">暂无子分类</div>
        </div>
      </div>
    </div>
