<script setup lang="ts">
  ...
  import type { BudgetCreate, FamilyMember, Transaction } from '@/schema/money';
  ...
  async function loadLedgerTransactions(serialNum: string) {
    ...
    try {
      ...
      allTransactions.value = results.filter((transaction): transaction is Transaction =>
        Boolean(transaction),
      );
    } catch (error) {
      console.error(error);
    ...
    <template v-else>
      <div v-if="currentLedger" class="flex flex-col gap-6 max-w-7xl mx-auto">
        <header class="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          <button
            class="inline-flex items-center gap-2 px-3 py-2 border border-blue-200 dark:border-blue-800 rounded-lg bg-white dark:bg-gray-800 cursor-pointer text-gray-900 dark:text-gray-100 hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors"
            @click="goBack"
          >
            ...
            >
              <div class="flex items-center gap-2">
                <button
                  v-for="tab in tabs"
                  :key="tab.key"
                  class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border transition-all text-xs"
                  :class="[
                    activeTab === tab.key
              ...
              <div class="flex flex-wrap gap-4 mt-3 text-sm text-gray-600 dark:text-gray-400">
                <span
                  class="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 rounded-md font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100"
                >
                  <LucideCoins class="w-4 h-4 shrink-0 text-blue-600 dark:text-blue-400" />
                  {{ currentLedger.baseCurrency?.code || 'CNY' }}
                </span>
                <span
                  class="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 rounded-md font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100"
                >
                  <LucideCalendarClock class="w-4 h-4 shrink-0 text-blue-600 dark:text-blue-400" />
                  {{ getSettlementCycleName(currentLedger.settlementCycle) }}
                </span>
                <span
                  class="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 rounded-md font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100"
                >
                  <LucideUsers class="w-4 h-4 shrink-0 text-blue-600 dark:text-blue-400" />
                  {{ memberCount }}
                </span>
                <span
                  class="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 rounded-md font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100"
                >
                  <LucideWallet class="w-4 h-4 shrink-0 text-blue-600 dark:text-blue-400" />
                  {{ accountCount }}
                </span>
                <span
                  class="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 rounded-md font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100"
                >
                  <LucideBarChart3 class="w-4 h-4 shrink-0 text-blue-600 dark:text-blue-400" />
                  {{ activeTransactions }}
                </span>
              </div>
            </div>
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <article
            class="flex items-center gap-4 p-4 rounded-2xl bg-gray-100 dark:bg-gray-800 shadow-sm border-l-4 border-green-500"
          >
            <LucideTrendingUp class="w-8 h-8 text-gray-600 dark:text-gray-400" />
            <div>
              <p class="m-0 text-gray-600 dark:text-gray-400">总收入</p>
          ...
          <article
            class="flex items-center gap-4 p-4 rounded-2xl bg-gray-100 dark:bg-gray-800 shadow-sm border-l-4 border-red-500"
          >
            <LucideTrendingDown class="w-8 h-8 text-gray-600 dark:text-gray-400" />
            <div>
              <p class="m-0 text-gray-600 dark:text-gray-400">总支出</p>
              <h3 class="m-0 mt-1 text-2xl text-gray-900 dark:text-gray-100">
          ...
          <article
            class="flex items-center gap-4 p-4 rounded-2xl bg-gray-100 dark:bg-gray-800 shadow-sm border-l-4 border-gray-500"
          >
            <LucideWallet class="w-8 h-8 text-gray-600 dark:text-gray-400" />
            <div>
              <p class="m-0 text-gray-600 dark:text-gray-400">净余额</p>
              <h3 class="m-0 mt-1 text-2xl text-gray-900 dark:text-gray-100">
          ...
          <article
            class="flex items-center gap-4 p-4 rounded-2xl bg-gray-100 dark:bg-gray-800 shadow-sm border-l-4 border-yellow-500"
          >
            <LucideReceiptText class="w-8 h-8 text-gray-600 dark:text-gray-400" />
            <div>
              <p class="m-0 text-gray-600 dark:text-gray-400">待结算</p>
              <h3 class="m-0 mt-1 text-2xl text-gray-900 dark:text-gray-100">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-[minmax(280px,360px)_1fr] gap-6 mt-8">
          <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
            <div class="flex justify-between items-center">
              <h2 class="m-0 text-xl text-gray-900 dark:text-gray-100">成员概览</h2>
            ...
            >
              ...
              >
                <div class="flex items-center gap-3">
                  <div
                    class="w-13 h-13 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center font-bold text-xl text-white shrink-0 shadow-sm"
                  >
                    {{ member.name.charAt(0).toUpperCase() }}
                  </div>
                  <div>
                    <p
                      class="m-0 font-bold text-base text-gray-900 dark:text-gray-100 tracking-tight"
                    >
                      {{ member.name }}
                    </p>
                    <p
                      class="m-0 mt-1.5 text-xs text-gray-600 dark:text-gray-400 font-medium px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded inline-block w-fit"
                    >
                ...
                <div
                  class="grid grid-cols-3 gap-4 mt-5 pt-4 border-t border-gray-200 dark:border-gray-700"
                >
                  <div
                    class="flex flex-col items-center gap-1 px-2 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:-translate-y-px min-w-0"
                  >
                    <span
                      class="text-[9px] text-gray-600 dark:text-gray-400 font-semibold tracking-wider whitespace-nowrap text-center"
                      >交易笔数</span
                    >
                    <strong
                      class="text-xs font-bold text-gray-900 dark:text-gray-100 tracking-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-full"
                      >{{ member.transactionCount ?? 0 }}</strong
                    >
                  </div>
                  <div
                    class="flex flex-col items-center gap-1 px-2 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:-translate-y-px min-w-0"
                  >
                    ...
                    <strong
                      class="text-xs font-bold text-gray-900 dark:text-gray-100 tracking-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-full"
                      >{{ formatCurrency(member.totalPaid) }}</strong
                    >
                  </div>
                  <div
                    class="flex flex-col items-center gap-1 px-2 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 transition-all hover:bg-gray-100 dark:hover:bg-gray-700 hover:-translate-y-px min-w-0"
                  >
                    ...
                    <strong
                      class="text-xs font-bold text-gray-900 dark:text-gray-100 tracking-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-full"
                      >{{ formatCurrency(member.totalOwed) }}</strong
                    >
                  </div>
                </div>
              </article>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
            ...
            <div
              class="max-h-[360px] overflow-y-auto overflow-x-hidden rounded-lg [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
            >
              <TransactionTable
