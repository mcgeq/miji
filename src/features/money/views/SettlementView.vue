...
<template>
  <div class="p-0">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
      <h2 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">结算中心</h2>
      <div
        v-if="currentLedger"
    ...
    <div class="min-h-[400px]">
      <div v-if="activeTab === 'overview'" class="animate-fade-in">
        ...
        <div v-else class="flex flex-col items-center justify-center p-12 text-center">
          ...
          <p class="text-gray-600 dark:text-gray-400">需要选择一个家庭账本才能查看结算信息</p>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'records'" class="animate-fade-in">
        ...
        <div v-else class="flex flex-col items-center justify-center p-12 text-center">
          <LucideAlertCircle class="w-12 h-12 text-yellow-500 dark:text-yellow-400 mb-4" />
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">请先选择账本</h3>
          <p class="text-gray-600 dark:text-gray-400">需要选择一个家庭账本才能查看结算记录</p>
        </div>
      </div>
    </div>
  </div>
