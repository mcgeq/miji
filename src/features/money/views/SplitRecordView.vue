<script setup lang="ts">
  ...
  const filteredRecords = computed(() => {
    ...
    if (searchKeyword.value) {
      ...
      result = result.filter(record => record.serialNum.toLowerCase().includes(keyword));
    }
  ...
  const statistics = computed(() => {
    ...
    const completed = filteredRecords.value.filter(r => r.splitDetails.every(d => d.isPaid)).length;
...
<template>
  <div class="flex flex-col gap-6 p-8 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        <h1 class="text-3xl font-semibold text-gray-900 dark:text-gray-100 mb-2">分摊记录</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400">查看和管理所有费用分摊记录</p>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        class="flex items-center gap-4 p-6 bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-700 rounded-xl shadow-sm"
      >
        <LucideUsers class="w-8 h-8 text-gray-700 dark:text-gray-300 shrink-0" />
        <div class="flex flex-col gap-1">
          <label class="text-sm text-gray-600 dark:text-gray-400">总记录数</label>
          <strong class="text-2xl font-semibold text-gray-900 dark:text-gray-100"
      ...
      <div
        class="flex items-center gap-4 p-6 bg-green-100 dark:bg-green-900/30 border-2 border-green-600 dark:border-green-700 rounded-xl shadow-sm"
      >
        <LucideCheck class="w-8 h-8 text-green-600 dark:text-green-400 shrink-0" />
        <div class="flex flex-col gap-1">
          <label class="text-sm text-gray-600 dark:text-gray-400">已完成</label>
          <strong class="text-2xl font-semibold text-gray-900 dark:text-gray-100"
            >{{ statistics.completed }}</strong
          >
        </div>
      </div>

      <div
        class="flex items-center gap-4 p-6 bg-yellow-100 dark:bg-yellow-900/30 border-2 border-yellow-500 dark:border-yellow-700 rounded-xl shadow-sm"
      >
        <LucideClock class="w-8 h-8 text-yellow-600 dark:text-yellow-400 shrink-0" />
        <div class="flex flex-col gap-1">
          <label class="text-sm text-gray-600 dark:text-gray-400">进行中</label>
          <strong class="text-2xl font-semibold text-gray-900 dark:text-gray-100"
            >{{ statistics.pending }}</strong
          >
        </div>
      </div>

      <div
        class="flex items-center gap-4 p-6 bg-blue-100 dark:bg-blue-900/30 border-2 border-blue-600 dark:border-blue-700 rounded-xl shadow-sm"
      >
        <LucideCalendar class="w-8 h-8 text-blue-600 dark:text-blue-400 shrink-0" />
        <div class="flex flex-col gap-1">
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-center">
      ...
      <button
        class="relative flex items-center gap-2 px-6 py-3 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-sm transition-all"
        :class="[
      ...
      >
        ...
        筛选
        <span
          v-if="filterStatus !== 'all' || filterMember"
          class="absolute -top-2 -right-2 w-5 h-5 bg-blue-600 text-white rounded-full text-xs flex items-center justify-center"
        >
    ...
    <div
      v-if="showFilters"
      class="flex flex-col sm:flex-row gap-4 p-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-700"
    >
      <div class="flex-1 flex flex-col gap-2">
        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">状态</label>
        <select
          v-model="filterStatus"
          class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
        >
      ...
      <div class="flex items-end">
        <button
          class="px-4 py-2 bg-transparent border border-gray-300 dark:border-gray-600 rounded-md text-sm transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
          @click="resetFilters"
        >
          重置
        </button>
    ...
    <div v-if="loading" class="flex flex-col items-center gap-4 py-12">
      <div
        class="w-8 h-8 border-3 border-gray-300 dark:border-gray-600 border-t-blue-600 rounded-full animate-spin"
      />
    ...
    <div v-else-if="filteredRecords.length > 0" class="flex flex-col gap-4">
      ...
      >
        <div class="flex justify-between items-center mb-4">
          <div class="flex flex-col gap-1">
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">分摊记录</h3>
            <span class="text-xs text-gray-500 dark:text-gray-400"
              >{{ formatDate(record.createdAt) }}</span
            >
          </div>
          <div
            class="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium"
            :class="[
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <strong class="text-xl font-semibold text-blue-600 dark:text-blue-400"
              >{{ formatAmount(record.totalAmount) }}</strong
            >
          </div>
          ...
          <div>
            ...
            <div class="flex items-center">
              ...
              <div
                v-if="record.splitDetails.length > 3"
                class="w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-300 flex items-center justify-center text-xs font-semibold border-2 border-white dark:border-gray-800 -ml-2"
              >
          ...
          <div>
            ...
            <span class="text-xs text-gray-600 dark:text-gray-400">
              {{ record.splitDetails.filter(d => d.isPaid).length }}/
              {{ record.splitDetails.length }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div v-else class="flex flex-col items-center gap-4 py-16 text-center">
      ...
      <p class="text-lg text-gray-600 dark:text-gray-400">暂无分摊记录</p>
      <span class="text-sm text-gray-400 dark:text-gray-500"
        >创建交易并启用分摊后，记录将显示在这里</span
      >
    </div>
  </div>
</template>
