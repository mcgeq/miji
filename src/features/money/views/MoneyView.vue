<script setup lang="ts">
  ...
  watchEffect(() => {
    if (hasModalOpen.value) {
      ...
      if (scrollY) window.scrollTo(0, Number.parseInt(scrollY || '0', 10) * -1);
    }
  });
  ...
  async function finalizeChange(
    closeFn: () => void,
    refreshFns: Array<() => Promise<void> | void>,
  ) {
    closeFn();
  ...
  function finalizeBudgetChange() {
    return finalizeChange(closeBudgetModal, [async () => budgetListRef.value?.refresh()]);
  }

  function finalizeReminderChange() {
    return finalizeChange(closeReminderModal, [async () => reminderListRef.value?.refresh()]);
  }
  ...
  // ------------------ Card Events ------------------
  function handleCardChange(index: number, card: CardData) {
    if (hasModalOpen.value) return;
    Lg.d('handleCardChange', '卡片切换:', index, card.title);
  }
  function handleCardClick(_index: number, _card: CardData) {}
  ...
  onMounted(async () => {
    ...
    // 监听分期处理失败事件
    unlistenFailed = await listen<InstallmentProcessFailedEvent>(
...
<template>
  <div class="w-full max-w-7xl mx-auto p-5 md:p-8 overflow-x-hidden">
    ...
    <!-- 快捷操作 & Tabs -->
    <div
      class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden mt-0.5 w-full max-w-[55rem] mx-auto"
    >
      <div
        class="flex justify-center items-center px-3 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"
      >
        <div class="flex flex-wrap justify-center gap-2 md:gap-3">
          <button
            class="shrink-0 flex items-center justify-center gap-1.5 px-3 md:px-4 py-2 text-sm font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 border-none rounded-md cursor-pointer transition-opacity hover:opacity-80"
            @click="showAccountModal"
          >
            <LucideCreditCard class="w-4 h-4 shrink-0" />
            <span class="whitespace-nowrap hidden sm:inline"
              >{{ t('financial.quickActions.account') }}</span
            >
          </button>
          <button
            class="shrink-0 flex items-center justify-center gap-1.5 px-3 md:px-4 py-2 text-sm font-medium bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 border-none rounded-md cursor-pointer transition-opacity hover:opacity-80"
            @click="showTransactionModal(TransactionTypeSchema.enum.Income)"
          >
            <LucidePlusCircle class="w-4 h-4 shrink-0" />
            <span class="whitespace-nowrap hidden sm:inline"
              >{{ t('financial.quickActions.income') }}</span
            >
          </button>
          <button
            class="shrink-0 flex items-center justify-center gap-1.5 px-3 md:px-4 py-2 text-sm font-medium bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-none rounded-md cursor-pointer transition-opacity hover:opacity-80"
            @click="showTransactionModal(TransactionTypeSchema.enum.Expense)"
          >
            <LucideMinusCircle class="w-4 h-4 shrink-0" />
            <span class="whitespace-nowrap hidden sm:inline"
              >{{ t('financial.quickActions.expense') }}</span
            >
          </button>
          <button
            class="shrink-0 flex items-center justify-center gap-1.5 px-3 md:px-4 py-2 text-sm font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border-none rounded-md cursor-pointer transition-opacity hover:opacity-80"
            @click="showTransactionModal(TransactionTypeSchema.enum.Transfer)"
          >
            <LucideArrowRightLeft class="w-4 h-4 shrink-0" />
            <span class="whitespace-nowrap hidden sm:inline"
              >{{ t('financial.quickActions.transfer') }}</span
            >
          </button>
          <button
            class="shrink-0 flex items-center justify-center gap-1.5 px-3 md:px-4 py-2 text-sm font-medium bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border-none rounded-md cursor-pointer transition-opacity hover:opacity-80"
            @click="showBudgetModal"
          >
            <LucideTarget class="w-4 h-4 shrink-0" />
            <span class="whitespace-nowrap hidden sm:inline"
              >{{ t('financial.quickActions.budget') }}</span
            >
          </button>
          <button
            class="shrink-0 flex items-center justify-center gap-1.5 px-3 md:px-4 py-2 text-sm font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 border-none rounded-md cursor-pointer transition-opacity hover:opacity-80"
            @click="showReminderModal"
          >
            <LucideBell class="w-4 h-4 shrink-0" />
            <span class="whitespace-nowrap hidden sm:inline"
              >{{ t('financial.quickActions.reminder') }}</span
            >
          </button>
        </div>
      </div>
      <!-- Tabs -->
      <div
        class="flex overflow-x-auto border-t border-gray-300 dark:border-gray-700 justify-center bg-white dark:bg-gray-800"
      >
        <button
          v-for="tab in tabs"
          :key="tab.key"
          class="shrink-0 px-6 py-3 text-sm font-medium border-b-3 cursor-pointer transition-all"
          :class="[
            activeTab === tab.key
