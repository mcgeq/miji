<script setup lang="ts">
  ...
  function calculateCycleFromPrevious(record: PeriodRecords) {
    ...
    const cycleDays = Math.ceil((current.getTime() - previous.getTime()) / (1000 * 60 * 60 * 24));
  ...
  <Card shadow="md" padding="md">
    ...
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <Button variant="primary" size="sm" @click="emit('addRecord')">
          <Plus class="w-5 h-5" />
        </Button>
      </div>
    </template>
    ...
    >
      <div
        class="flex items-center justify-center w-16 h-16 mb-3 rounded-full bg-gray-200 dark:bg-gray-700"
      >
    ...
    <div v-else class="flex flex-col gap-3">
      ...
      >
        <!-- 日期显示 -->
        <div class="flex flex-col items-center justify-center min-w-[3rem] shrink-0">
        ...
        >
          ...
          <div
            class="inline-flex items-center self-start sm:self-auto px-2 py-1 text-xs font-medium rounded bg-blue-600 dark:bg-blue-500 text-white"
          >
        ...
        <!-- 箭头图标 -->
        <ChevronRightCircle
          class="w-6 h-6 shrink-0 text-blue-600 dark:text-blue-400 transition-colors group-hover:text-blue-700 dark:group-hover:text-blue-300"
        />
      </div>
    </div>
  </Card>
