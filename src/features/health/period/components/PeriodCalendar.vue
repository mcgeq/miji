<script setup lang="ts">
  ...
  function getCellClasses(day: CalendarDay): string {
    ...
    if (day.isToday) {
      baseClasses.push(
        'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 font-semibold rounded-md',
      );
    }
    ...
    if (hasPredicted) {
      baseClasses.push(
        "before:content-[''] before:absolute before:inset-0 before:border-2 before:border-dashed before:rounded-md before:pointer-events-none",
      );

      if (hasPredictedPeriod) {
        baseClasses.push(
          'before:border-green-300 dark:before:border-green-600 bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-400',
        );
      }
      if (hasPredictedOvulation) {
        baseClasses.push(
          'before:border-red-300 dark:before:border-red-600 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-400',
        );
      }
      if (hasPredictedFertile) {
        baseClasses.push(
          'before:border-orange-300 dark:before:border-orange-600 bg-orange-50 dark:bg-orange-950 text-orange-700 dark:text-orange-400',
        );
      }
    }
  ...
  function getEventDotClass(event: PeriodCalendarEvent): string {
    ...
    if (event.isPredicted) {
      baseClasses.push(
        'border-2 border-white dark:border-gray-800 bg-[repeating-linear-gradient(45deg,transparent,transparent_1px,currentColor_1px,currentColor_2px)]',
      );
    }
  ...
  function getEventBadgeClass(event: PeriodCalendarEvent): string {
    const baseClasses = [
      'py-1 px-2 rounded-md text-xs font-semibold shadow-sm transition-all duration-150 hover:scale-105',
    ];
  ...
  function constrainToViewport(
    ...
    if (constrainedX < scrollX + 10) {
      constrainedX = scrollX + 10;
    } else if (constrainedX + tooltipWidth > scrollX + viewportWidth - 10) {
      constrainedX = scrollX + viewportWidth - tooltipWidth - 10;
    }

    if (constrainedY < scrollY + 10) {
      constrainedY = scrollY + 10;
    } else if (constrainedY + tooltipHeight > scrollY + viewportHeight - 10) {
      constrainedY = scrollY + viewportHeight - tooltipHeight - 10;
    }
  ...
  function generateExtraInfo(day: CalendarDay): string | undefined {
    ...
    const hasFertile = day.events.some(e => e.type === 'fertile' || e.type === 'predicted-fertile');
    const hasPeriod = day.events.some(e => e.type === 'period' || e.type === 'predicted-period');

    if (hasOvulation) return '建议: 避孕用户需特别注意，备孕用户的最佳时机';
    if (hasFertile) return '建议: 仍有怀孕可能，请注意避孕措施';
    if (hasPeriod) return '建议: 注意休息，保持温暖，适量运动';
    return undefined;
  }
...
<template>
  <div
    class="h-full flex flex-col bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md"
  >
    <!-- 头部导航 -->
    <div
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        >
          <i
            :class="viewMode === 'calendar' ? 'i-tabler-list' : 'i-tabler-calendar'"
            class="wh-3 mr-1"
          />
    ...
    <div
      ...
      class="flex flex-col bg-gray-50 dark:bg-gray-900 px-2 pt-2 pb-0"
    >
      <!-- 星期标题 -->
      ...
      <div class="grid grid-cols-7 mb-0">
        ...
        >
          ...
          <div
            ...
            class="absolute bottom-0 left-1/2 -translate-x-1/2 flex gap-0.5"
          >
            <div
    ...
    <div
      v-else
      class="flex-1 p-3 flex flex-col gap-2 overflow-y-auto bg-white dark:bg-gray-900 rounded-lg scrollbar-hide"
    >
      <div
      ...
      <div
        v-if="calendarDays.filter(d => d.events.length > 0).length === 0"
        class="flex flex-col items-center justify-center py-8 text-gray-500"
      >
        <i class="i-tabler-calendar-x text-gray-400 wh-8" />
        <p class="text-sm mt-2">本月暂无记录</p>
      </div>
    </div>

    <!-- 图例 -->
    <div
      class="flex items-center justify-center gap-4 px-3 py-1.5 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 max-sm:gap-2 max-sm:py-1.5 max-sm:flex-wrap"
    >
      ...
      <div class="flex items-center gap-1.5">
        <div
          class="w-2 h-2 rounded-full border-2 border-white dark:border-gray-800 bg-[repeating-linear-gradient(45deg,transparent,transparent_1px,#22c55e_1px,#22c55e_2px)]"
        />
      ...
      <div class="flex items-center gap-1.5">
        <div
          class="w-2 h-2 rounded-full border-2 border-white dark:border-gray-800 bg-[repeating-linear-gradient(45deg,transparent,transparent_1px,#ef4444_1px,#ef4444_2px)]"
        />
      ...
      <div class="flex items-center gap-1.5">
        <div
          class="w-2 h-2 rounded-full border-2 border-white dark:border-gray-800 bg-[repeating-linear-gradient(45deg,transparent,transparent_1px,#f97316_1px,#f97316_2px)]"
        />
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 min-w-[250px] max-w-[320px] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25),0_0_0_1px_rgba(0,0,0,0.05)] dark:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.1)] relative z-1"
      >
        <!-- 日期标题 -->
        <div class="flex items-center justify-center mb-2">
          <span class="text-sm font-semibold">{{ tooltip.date }}</span>
          <span
            v-if="tooltip.isToday"
            class="py-0.5 px-2 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 text-xs rounded-full ml-2"
            >今天</span
          >
        </div>
        ...
        <div class="flex flex-col gap-2 text-center">
          ...
          >
            ...
            <div class="flex-1 text-left max-w-[200px]">
              ...
              <span
                v-if="getRiskLevel(event)"
                :class="getTooltipRiskClass(event)"
        ...
        <div v-if="tooltip.extraInfo" class="mt-2 text-left">
          ...
          <div class="text-xs leading-relaxed opacity-75">{{ tooltip.extraInfo }}</div>
        </div>
      </div>
    </div>
  </Teleport>
