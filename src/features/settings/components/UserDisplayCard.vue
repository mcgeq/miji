...
<template>
  <div class="w-full">
    ...
    <div class="block lg:hidden">
      <div
        class="relative p-4 rounded-2xl bg-gradient-to-br from-purple-600 to-blue-600 text-white shadow-lg overflow-hidden"
      >
        <!-- 简化的背景装饰 -->
        <div class="absolute inset-0 pointer-events-none">
          <div
            class="absolute inset-0 opacity-30"
            style="background-image: url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%23ffffff\' fill-opacity=\'0.08\'%3E%3Ccircle cx=\'30\' cy=\'30\' r=\'2\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')"
          />
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="relative shrink-0 group">
              <div class="relative w-12 h-12 transform transition-transform group-hover:scale-105">
                <!-- 头像光环 -->
                <div
                  class="absolute -inset-0.5 bg-gradient-to-r from-white/30 to-white/10 rounded-full blur-sm"
                />
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <h3
                  class="text-lg font-bold bg-gradient-to-r from-white to-white/90 bg-clip-text text-transparent truncate"
                >
                  {{ user?.name || '用户' }}
                </h3>
                <span
                  class="shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full uppercase tracking-wide bg-white/20 backdrop-blur-sm transition-transform hover:scale-105"
                >
                  {{ getRoleText(user?.role) }}
                </span>
              </div>
              <p class="text-sm text-white/80 font-medium truncate">{{ user?.email || '' }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="hidden lg:block">
      <div
        class="relative p-8 rounded-3xl bg-gradient-to-br from-purple-600 to-blue-600 text-white overflow-hidden"
        style="box-shadow: 0 25px 50px -12px rgba(147, 51, 234, 0.2)"
      >
        <!-- 动态背景装饰 -->
        <div class="absolute inset-0 pointer-events-none">
          <!-- 网格图案 -->
        ...
        <div class="relative z-10">
          ...
          <div class="flex items-center gap-6 mb-10">
            ...
            <div class="relative shrink-0 group">
              <div class="relative w-16 h-16 transform transition-transform group-hover:scale-105">
                <!-- 头像光环 -->
                <div
                  class="absolute -inset-1 bg-gradient-to-r from-white/30 to-white/10 rounded-full blur-md"
                />
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-3 mb-1">
                <h2
                  class="text-2xl font-bold bg-gradient-to-r from-white to-white/90 bg-clip-text text-transparent truncate"
                >
                  {{ user?.name || '用户' }}
                </h2>
                <span
                  class="shrink-0 text-xs font-semibold px-3 py-1.5 rounded-full uppercase tracking-wide bg-white/20 backdrop-blur-md transition-transform hover:scale-105"
                >
                  {{ getRoleText(user?.role) }}
                </span>
              </div>
              <p class="text-base text-white/80 font-medium truncate">{{ user?.email || '' }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div
              class="relative p-6 text-center rounded-2xl bg-white/10 backdrop-blur-xl overflow-hidden transition-all hover:bg-white/15 hover:shadow-lg hover:scale-105 group"
            >
              <div
              ...
              <div class="relative z-10">
                ...
                <div class="text-white font-bold px-1 wrap-break-words">
