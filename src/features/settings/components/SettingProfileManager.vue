...
async function exportProfile(profile: SettingProfile) {
  try {
    const data = await invokeCommand<any>('user_setting_profile_export', {
      profileSerialNum: profile.serialNum,
    });
  ...
  <div class="max-w-4xl w-full">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        <!-- 导入按钮 -->
        <label class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer flex items-center gap-2">
    ...
    <div v-else-if="profiles.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-500">
              {{ $t('settings.profiles.createdAt') || '创建于' }}: {{ new Date(profile.createdAt).toLocaleString() }}
            </p>
          </div>
