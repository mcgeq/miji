<script setup lang="ts">
  ...
  import { useTodoAssociations } from '../../composables/useTodoAssociations';
  ...
  // 项目和标签关联管理
  const { selectedProjects, selectedTags, addProject, removeProject, addTag, removeTag } =
    useTodoAssociations(props.todo.serialNum);

  // 时间进度追踪
  const {
    timeProgress,
    urgency,
    refresh: refreshTimeProgress,
  } = useTimeProgress(todoCopy.value.createdAt, todoCopy.value.dueAt);
  ...
  // 计算属性
  const completed = computed(() => todoCopy.value.status === StatusSchema.enum.Completed);
  const showMenu = computed(() => menuStore.getMenuSerialNum === todoCopy.value.serialNum);
  // 判断是否有modal打开
  const hasModalOpen = computed(() => {
    return (
      ...
      showMenu.value ||
      showEditModal.value ||
      showDueDateModal.value ||
      showEditRepeatModal.value ||
      !!currentPopup.value
    );
  });
...
<template>
  ...
  >
    ...
    <div class="flex justify-between items-center flex-1 w-full max-w-full min-w-0 overflow-hidden">
      <div
        class="flex items-center gap-2 md:gap-2 flex-1 min-w-0 max-w-full pl-0.5 md:pl-2 overflow-hidden"
      >
        <PriorityBadge
...
<style scoped>
  ...
  .priority-gradient-low {
    background: linear-gradient(
      ...
      color-mix(in srgb, rgb(16, 185, 129) light-dark(3%, 5%), transparent)
    ); /* emerald-500 */
  }
  ...
  .priority-gradient-medium {
    background: linear-gradient(
      to bottom right,
      light-dark(oklch(94% 0.015 90), color-mix(in srgb, oklch(20% 0.01 45) 98%, transparent)),
      color-mix(in srgb, rgb(245, 158, 11) light-dark(3%, 5%), transparent)
    ); /* amber-500 */
  }
  ...
  .priority-gradient-high {
    background: linear-gradient(
      to bottom right,
      light-dark(oklch(94% 0.015 90), color-mix(in srgb, oklch(20% 0.01 45) 98%, transparent)),
      color-mix(in srgb, rgb(239, 68, 68) light-dark(3%, 5%), transparent)
    ); /* red-500 */
  }
  ...
  .priority-gradient-urgent {
    background: linear-gradient(
      ...
      color-mix(in srgb, rgb(220, 38, 38) light-dark(5%, 8%), transparent)
    ); /* red-600 */
  }
