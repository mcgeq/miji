...
<template>
  <Modal :open="show" title="设置重复" size="lg" @close="emit('close')" @confirm="save">
    <div class="space-y-4">
      <!-- Repeat Type -->
      <div>
        ...
        <Select v-model="form.type" :options="repeatTypeOptions" full-width @change="resetForm" />
      </div>
      ...
      <div v-if="form.type === 'Daily'" class="space-y-2">
        ...
        <Input v-model="form.interval" type="number" required full-width />
      </div>
      ...
      <div v-if="form.type === 'Monthly'" class="space-y-3">
        ...
        <Input v-model="form.interval" type="number" required full-width />
        <Select v-model="form.day" :options="monthlyDayOptions" full-width />
      </div>
      ...
      <div v-if="form.type === 'Yearly'" class="space-y-3">
        ...
        <Input v-model="form.interval" type="number" required full-width />
        <Select v-model="form.month" :options="monthOptions" full-width />
        <Select v-model="form.day" :options="yearlyDayOptions" full-width />
      </div>
      ...
      <div v-if="form.type === 'Custom'" class="space-y-2">
        ...
        <Input v-model="form.description" type="text" required full-width />
      </div>
    </div>
  </Modal>
</template>
